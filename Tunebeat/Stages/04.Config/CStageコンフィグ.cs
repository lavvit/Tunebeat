	...
	internal class CStageコンフィグ : CStage
	{
		...
		public override int On進行描画()
		{
			...
			if(TJAPlayer3.Tx.Config_Background != null )
                TJAPlayer3.Tx.Config_Background.t2D描画( 0, 0 );
			//---------------------
			#endregion
			#region [ メニューカーソル ]
			//---------------------
			if( TJAPlayer3.Tx.Config_Cursor != null )
			{
				...
				int x = 110;
				int y = (int)(149.5 + (this.n現在のメニュー番号 * 37.5));
				for( int i = 0; i < (340.0 / (double)TJAPlayer3.Tx.Config_Cursor.szテクスチャサイズ.Width); i ++)
				{
                    TJAPlayer3.Tx.Config_Cursor.t2D描画( x, y);
					x += TJAPlayer3.Tx.Config_Cursor.szテクスチャサイズ.Width;
				}
			}
			//---------------------
			#endregion
			...
			for ( int i = 0; i < txMenuItemLeft.GetLength( 0 ); i++ )
			{
                ...
                txMenuItemLeft[i, flag].t2D描画(282 - (num4 / 2) + TJAPlayer3.Skin.Config_ItemText_Correction_X, menuY + TJAPlayer3.Skin.Config_ItemText_Correction_Y ); //55
				//txMenuItem.Dispose();
				menuY += stepY;
			}
			//---------------------
			...
			if ( !TJAPlayer3.EnumSongs.IsEnumerating || TJAPlayer3.actEnumSongs.bコマンドでの曲データ取得 != true )
			{
				...
				else if (TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_F) || TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_J) || TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_RETURN))
				{
				...
				this.ctキー反復用.Up.tキー反復( TJAPlayer3.Input管理.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_UP), new CCounter.DGキー処理( this.tカーソルを上へ移動する ) );

				if (TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_D))
				{
					this.tカーソルを上へ移動する();
				}
				this.ctキー反復用.Down.tキー反復( TJAPlayer3.Input管理.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_DOWN ), new CCounter.DGキー処理( this.tカーソルを下へ移動する ) );
