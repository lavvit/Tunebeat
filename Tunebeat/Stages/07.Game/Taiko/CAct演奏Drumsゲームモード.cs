...
namespace TJAPlayer3
{
    internal class CAct演奏Drumsゲームモード : CActivity
    {
        ...
        public override int On進行描画()
        {
            if( TJAPlayer3.ConfigIni.eGameMode == EGame.完走叩ききりまショー || TJAPlayer3.ConfigIni.eGameMode == EGame.完走叩ききりまショー激辛 )
            {
                //if( this.st叩ききりまショー.b最初のチップが叩かれた == true )//&&
                  //CDTXMania.stage演奏ドラム画面.r検索範囲内にチップがあるか調べる( TJAPlayer3.stage演奏ドラム画面.ct演奏用タイマー.db現在の値, 0, 3000 ) )
                ...
                if( this.st叩ききりまショー.bタイマー使用中 )
                {
				    ...
				    {
                        this.st叩ききりまショー.ct残り時間.t進行();
					    if( !TJAPlayer3.stage演奏ドラム画面.r検索範囲内にチップがあるか調べる( TJAPlayer3.stage演奏ドラム画面.ct演奏用タイマー.db現在の値, 0, 5000, 0 ) || this.st叩ききりまショー.b加算アニメ中 == true )
					    {
                            this.st叩ききりまショー.bタイマー使用中 = false;
                ...
                {
                    if ((this.st叩ききりまショー.b最初のチップが叩かれた == true && ( TJAPlayer3.stage演奏ドラム画面.r検索範囲内にチップがあるか調べる(TJAPlayer3.stage演奏ドラム画面.ct演奏用タイマー.db現在の値, 0, 2000, 0 ) ) ) )
                    {
                ...
                if(TJAPlayer3.Tx.Taiko_Combo != null )
                {
                    ...
                    TJAPlayer3.Tx.GameMode_Timer_Tick.fZ軸中心回転 = fRotate;
                    TJAPlayer3.Tx.GameMode_Timer_Tick.t2D中心基準描画(280 - 640, -(134 - 360));
        ...
        private void t叩ききりまショー_評価をして残り時間を延長する()
        {
            ...
            if( this.n最後に時間延長した時刻 + 11000 <= TJAPlayer3.stage演奏ドラム画面.ct演奏用タイマー.db現在の値 )
            {
                //1項目につき5秒
                //-精度
                ...
                this.n最後に時間延長した時刻 = (int)TJAPlayer3.stage演奏ドラム画面.ct演奏用タイマー.db現在の値;
                if( n延長する時間 < 0 )
                    n延長する時間 = 0;
