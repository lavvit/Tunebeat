		...
		public override int On進行描画()
		{
			if ( !base.b活性化してない )
			{
                ...
                */
                ...
                {
                    TJAPlayer3.Tx.Gauge_Base[0].t2D描画( 492, 144, new Rectangle( 0, 0, 700, 44 ) );
                }
                if( TJAPlayer3.stage演奏ドラム画面.bDoublePlay && TJAPlayer3.Tx.Gauge_Base[1] != null )
                {
                    TJAPlayer3.Tx.Gauge_Base[1].t2D描画( 492, 532, new Rectangle( 0, 0, 700, 44 ) );
                }
                #region[ ゲージ1P ]
                if( TJAPlayer3.Tx.Gauge[0] != null )
                {
                    TJAPlayer3.Tx.Gauge[0].t2D描画( 492, 144, new Rectangle( 0, 0, nRectX, 44 ) );
				...
				TJAPlayer3.Tx.Gauge_Rainbow[this.ct虹アニメ.n現在の値].t2D描画(492, 144);
                                TJAPlayer3.Tx.Gauge_Rainbow[虹ベース].Opacity = (ct虹透明度.n現在の値 * 255 / ct虹透明度.n終了値)/1;
                                TJAPlayer3.Tx.Gauge_Rainbow[虹ベース].t2D描画(492, 144);
                            }
                        }
                        TJAPlayer3.Tx.Gauge_Line[0].t2D描画( 492, 144 );
                    }
                    #region[ 「クリア」文字 ]
                    if( this.db現在のゲージ値[ 0 ] >= 80.0 )
                    {
                        TJAPlayer3.Tx.Gauge[0].t2D描画( 1038, 144, new Rectangle( 0, 44, 58, 24 ) );
                    }
                    else
                    {
                        TJAPlayer3.Tx.Gauge[0].t2D描画( 1038, 144, new Rectangle( 58, 44, 58, 24 ) );
                    }
                    #endregion
                }
                #endregion
                ...
                {
                    TJAPlayer3.Tx.Gauge[1].t2D描画( 492, 532, new Rectangle( 0, 0, nRectX2P, 44 ) );
			    ...
			    this.ct虹透明度.t進行Loop();
                            if (TJAPlayer3.Tx.Gauge_Rainbow[this.ct虹アニメ.n現在の値] != null)
                            {
                                ...
                                TJAPlayer3.Tx.Gauge_Rainbow[ct虹アニメ.n現在の値].t2D上下反転描画(492, 532);
                                TJAPlayer3.Tx.Gauge_Rainbow[虹ベース].Opacity = (ct虹透明度.n現在の値 * 255 / ct虹透明度.n終了値) / 1;
                                TJAPlayer3.Tx.Gauge_Rainbow[虹ベース].t2D上下反転描画(492, 532);
                            }
                        }
                        TJAPlayer3.Tx.Gauge_Line[1].t2D描画( 492, 532 );
                    }
                    #region[ 「クリア」文字 ]
                    if( this.db現在のゲージ値[ 1 ] >= 80.0 )
                    {
                        TJAPlayer3.Tx.Gauge[1].t2D描画( 1038, 554, new Rectangle( 0, 44, 58, 24 ) );
                    }
                    else
                    {
                        TJAPlayer3.Tx.Gauge[1].t2D描画( 1038, 554, new Rectangle( 58, 44, 58, 24 ) );
                    }
                    #endregion
                }
                #endregion
                ...
                if(TJAPlayer3.Tx.Gauge_Soul_Fire != null )
                {
                    ...
                    for( int i = 0; i < TJAPlayer3.ConfigIni.nPlayerCount; i++ )
                    {
                        ...
                        {
                            this.ct炎.t進行Loop();
                            TJAPlayer3.Tx.Gauge_Soul_Fire.t2D描画( 1112, nSoulFire[ i ], new Rectangle( 230 * ( this.ct炎.n現在の値 ), 0, 230, 230 ) );
                        }
                    }
                }
                if(TJAPlayer3.Tx.Gauge_Soul != null )
                {
                    ...
                    for( int i = 0; i < TJAPlayer3.ConfigIni.nPlayerCount; i++ )
                    {
                        ...
                        {
                            TJAPlayer3.Tx.Gauge_Soul.t2D描画( 1184, nSoulY[ i ], new Rectangle( 0, 0, 80, 80 ) );
                        }
                        else
                        {
                            TJAPlayer3.Tx.Gauge_Soul.t2D描画( 1184, nSoulY[ i ], new Rectangle( 0, 80, 80, 80 ) );
                        }
                    }
                }
                ...
                for( int d = 0; d < 32; d++ )
                {
                    if( this.st花火状態[d].b使用中 )
                    {
                        ...
                        //{
                        //    CDTXMania.Tx.Gauge_Soul_Explosion.t2D描画( CDTXMania.app.Det進行Loopice, 1140, 73, new Rectangle( this.st花火状態[d].ct進行.n現在の値 * 140, 0, 140, 180 ) );
                        //}
                        //if (CDTXMania.Tx.Notes != null)
                        //{
                            //CDTXMania.Tx.Notes.t2D中心基準描画(CDTXMania.app.Det進行Loopice, 1224, 162, new Rectangle(this.st花火状態[d].nLane * 130, 0, 130, 130));
                            //this.tx音符.color4 = System.Drawing.Color.FromArgb( 1.0f, 1.0f, 1.0f - (this.st花火状態[d].ct進行.n現在の値 / 10f) );
                ...
                for( int d = 0; d < 32; d++ )
                {
                    if (this.st花火状態2P[d].b使用中)
                    {
                        ...
                        //if(CDTXMania.Tx.Gauge_Soul_Explosion != null )
                        //{
                        //    CDTXMania.Tx.Gauge_Soul_Explosion.t2D描画( CDTXMania.app.Det進行Loopice, 1140, 468, new Rectangle( this.st花火状態2P[d].ct進行.n現在の値 * 140, 0, 140, 180 ) );
                        //}
                        //if (CDTXMania.Tx.Notes != null)
                        //{
                        //    CDTXMania.Tx.Notes.t2D中心基準描画(CDTXMania.app.Det進行Loopice, 1224, 162, new Rectangle(this.st花火状態[d].nLane * 130, 0, 130, 130));
                ...
                for (int i = 0; i < 3; i ++)
                TJAPlayer3.act文字コンソール.tPrint(60, 0 + (i * 20), C文字コンソール.Eフォント種別.白, this.dbゲージ増加量_Branch[i, 0].ToString());
                TJAPlayer3.act文字コンソール.tPrint(60,80, C文字コンソール.Eフォント種別.白, this.db現在のゲージ値[0].ToString());
