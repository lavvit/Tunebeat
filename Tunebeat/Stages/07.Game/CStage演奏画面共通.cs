...
//		Debug.WriteLine( "★Queue: remove " + Path.GetFileName( stm.csound.strファイル名 ));
	    ...
	    internal E判定 e指定時刻からChipのJUDGEを返す(double nTime, CDTX.CChip pChip)
	    {
		...
		private E判定 e指定時刻からChipのJUDGEを返すImpl(double nTime, CDTX.CChip pChip )
		{
			if ( pChip != null )
			{
                ...
                {
				    if ( ct演奏用タイマー.db現在の値 > pChip.n発声時刻ms && ct演奏用タイマー.db現在の値 < pChip.nノーツ終了時刻ms )
				    {
					    return E判定.Perfect;
				    }
                }
                ...
                {
				    if ( ct演奏用タイマー.db現在の値 >= pChip.n発声時刻ms - 17 && ct演奏用タイマー.db現在の値 < pChip.nノーツ終了時刻ms )
				    {
					    return E判定.Perfect;
				    }
                }
...
//Trace.TraceInformation( "NTime={0}, nChannel={1:x2}", nTime, nChannel );
        ...
        {
            //if( dbProcess_time >= pChip.n発声時刻ms && dbProcess_time < pChip.nノーツ終了時刻ms )
            if ((int)ct演奏用タイマー.db現在の値 >= pChip.n発声時刻ms && (int)ct演奏用タイマー.db現在の値 <= pChip.nノーツ終了時刻ms)
            {
                if( pChip.nRollCount == 0 )
                {
                    pChip.b可視 = false;
		...
		protected E判定 tチップのヒット処理( double nHitTime, CDTX.CChip pChip )
		{
			return tチップのヒット処理( nHitTime, pChip, true );
		}
		protected abstract E判定 tチップのヒット処理(double nHitTime, CDTX.CChip pChip, bool bCorrectLane );
		protected E判定 tチップのヒット処理(double nHitTime, CDTX.CChip pChip, E楽器パート screenmode )		// E楽器パート screenmode
		{
			return tチップのヒット処理( nHitTime, pChip, screenmode, true, 1, 0 );
		}
		protected E判定 tチップのヒット処理(double nHitTime, CDTX.CChip pChip, E楽器パート screenmode, bool bCorrectLane, int nNowInput )
		{
			return tチップのヒット処理( nHitTime, pChip, screenmode, bCorrectLane, nNowInput, 0 );
		}
		protected unsafe E判定 tチップのヒット処理(double nHitTime, CDTX.CChip pChip, E楽器パート screenmode, bool bCorrectLane, int nNowInput, int nPlayer )
		{
				...
				case E楽器パート.TAIKO:
					{
                        ...
                        if ( pChip.nチャンネル番号 == 0x15 || pChip.nチャンネル番号 == 0x16 )
                        {
                            ...
                            if( bAutoPlay )
                            {
                                ...
                                {
                                    if( ct演奏用タイマー.db現在の値 > ( pChip.n発声時刻ms + ( 1000.0 / 15.0 ) * pChip.nRollCount ) )
                                    {
                                        if( this.nHand[ nPlayer ] == 0 )
                                            this.nHand[ nPlayer ]++;
                                        else
                                        ...
                                        this.tRollProcess( pChip, ct演奏用タイマー.db現在の値, 1, 0, 0, nPlayer );
                                    }
                                }
                            }
                            else
                            {
                                this.eRollState = E連打State.roll;
                                this.tRollProcess( pChip, ct演奏用タイマー.db現在の値, 1, nNowInput, 0, nPlayer );
                            }

                            //if ((int)ct演奏用タイマー.db現在の値 >= pChip.nノーツ終了時刻ms)
                            //{
                        ...
                        else if( pChip.nチャンネル番号 == 0x17 )
                        {
                            ...
                            if( bAutoPlay )
                            {
                                if( pChip.nBalloon != 0 )
                                {
                                    if( ct演奏用タイマー.db現在の値 > ( pChip.n発声時刻ms + ( ( pChip.nノーツ終了時刻ms - pChip.n発声時刻ms ) / pChip.nBalloon ) * pChip.nRollCount ) )
                                    {
                                        if( this.nHand[ nPlayer ] == 0 )
                                            this.nHand[ nPlayer ]++;
                                        else
                                        ...
                                        this.tBalloonProcess( pChip, ct演奏用タイマー.db現在の値, nPlayer );
                                    }
                                }
                            }
                            else
                        ...
                        {
                            if( pChip.nノーツ終了時刻ms <= ct演奏用タイマー.db現在の値 )
                            {
                                this.b連打中[ nPlayer ] = false;
                                //this.actChara.b風船連打中 = false;
                                pChip.bHit = true;
            ...
            if ((pChip.e楽器パート != E楽器パート.UNKNOWN))
            {
                ...
                {
                    actGauge.Damage(screenmode, pChip.e楽器パート, eJudgeResult, nPlayer, pChip.nコース);
                }
            }
            ...
            if ( eJudgeResult != E判定.Poor && eJudgeResult != E判定.Miss )
            {
                ...
                if ((int)actGauge.db現在のゲージ値[nPlayer] >= 100 && this.bIsAlreadyMaxed[nPlayer] == false)
                {
                    if(TJAPlayer3.Skin.Game_Chara_Ptn_SoulIn != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.b終了値に達した)
                    {
                        ...
                        this.actChara.ctキャラクターアクション_魂MAX = new CCounter(0, TJAPlayer3.Skin.Game_Chara_Ptn_SoulIn - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_SoulIn) * 2, null);
                ...
                if ((int)actGauge.db現在のゲージ値[nPlayer] >= 80 && this.bIsAlreadyCleared[nPlayer] == false)
                {
                    ...
                    {
                        this.actChara.アクションタイマーリセット();
                        this.actChara.ctキャラクターアクション_ノルマ = new CCounter(0, TJAPlayer3.Skin.Game_Chara_Ptn_ClearIn - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_ClearIn) * 2, null);
                            ...
                            if (!pChip.bGOGOTIME) //2018.03.11 kairera0467 チップに埋め込んだフラグから読み取る
                            {
                                if (TJAPlayer3.Skin.Game_Chara_Ptn_10combo != 0 && nPlayer == 0 && !this.actChara.ctキャラクターアクション_ノルマ.b進行中db && actChara.CharaAction_Balloon_Delay.b終了値に達した)
                                {
                                    if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] < 100)
                                    {
                                        ...
                                        // ジャンプ_ノーマル
                                        this.actChara.アクションタイマーリセット();
                                        this.actChara.ctキャラクターアクション_10コンボ = new CCounter(0, TJAPlayer3.Skin.Game_Chara_Ptn_10combo - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_10combo) * 2, null);
                                ...
                                if (TJAPlayer3.Skin.Game_Chara_Ptn_10combo_Max != 0 && nPlayer == 0 && !this.actChara.ctキャラクターアクション_魂MAX.b進行中db && actChara.CharaAction_Balloon_Delay.b終了値に達した)
                                {
                                    if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] >= 100)
                                    {
                                        ...
                                        // ジャンプ_MAX
                                        this.actChara.アクションタイマーリセット();
                                        this.actChara.ctキャラクターアクション_10コンボMAX = new CCounter(0, TJAPlayer3.Skin.Game_Chara_Ptn_10combo_Max - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_10combo_Max) * 2, null);
		...
		protected void tチップのヒット処理_BadならびにTight時のMiss( E楽器パート part, E楽器パート screenmode, int nコース)
		{
			this.tチップのヒット処理_BadならびにTight時のMiss( part, 0, screenmode, nコース);
		}
		protected void tチップのヒット処理_BadならびにTight時のMiss( E楽器パート part, int nLane, E楽器パート screenmode, int nコース)
		{
			...
			actGauge.Damage( screenmode, part, E判定.Miss, 0, nコース );
			switch ( part )
			{
				case E楽器パート.DRUMS:
				case E楽器パート.GUITAR:
...
//					nTimeDiff = Math.Abs( (int) ( nTime - nearestChip.n発声時刻ms ) );
        ...
        protected CDTX.CChip r指定時刻に一番近い未ヒットChip(double nTime, int nChannel, int nInputAdjustTime, int n検索範囲時間ms, int nPlayer )
		{
			//sw2.Start();
...
//					nTimeDiff = Math.Abs( (int) ( nTime - nearestChip.n発声時刻ms ) );
		...
		public bool r検索範囲内にチップがあるか調べる(double nTime, int nInputAdjustTime, int n検索範囲時間ms, int nPlayer )
		{
			nTime += nInputAdjustTime;
		...
		{
			var keyboard = TJAPlayer3.Input管理;
			if ( keyboard.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_F1) &&
				( keyboard.IsPushingKey( (int)DxLibDLL.DX.KEY_INPUT_LSHIFT) || keyboard.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_RSHIFT) ) )
			{	// shift+f1 (pause)
			...
			if ( ( !this.bPAUSE && ( base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED ) ) && ( base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) )
			{
				this.t入力処理_ドラム();
				 ...
				 if ( keyboard.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_UP ) )
				{	// UpArrow(scrollspeed up)
					ドラムスクロール速度アップ();
				}
				else if ( keyboard.IsPushedKey( (int)DxLibDLL.DX.KEY_INPUT_DOWN) )
				{	// DownArrow (scrollspeed down)
					ドラムスクロール速度ダウン();
				}
				else if ( keyboard.IsPushedKey( (int)DxLibDLL.DX.KEY_INPUT_DELETE) )
				{	// del (debug info)
					TJAPlayer3.ConfigIni.b演奏情報を表示する = !TJAPlayer3.ConfigIni.b演奏情報を表示する;
				}
				else if ( keyboard.IsPushedKey( (int)DxLibDLL.DX.KEY_INPUT_LEFT) )		// #24243 2011.1.16 yyagi UI for InputAdjustTime in playing screen.
				{
				}
				else if ( keyboard.IsPushedKey( (int)DxLibDLL.DX.KEY_INPUT_RIGHT) )		// #24243 2011.1.16 yyagi UI for InputAdjustTime in playing screen.
				{
				}
				else if ( ( base.eフェーズID == CStage.Eフェーズ.共通_通常状態 ) && ( keyboard.IsPushedKey( (int)DxLibDLL.DX.KEY_INPUT_ESCAPE ) ) && !this.actPauseMenu.bIsActivePopupMenu )
				{	// escape (exit)
                    this.t演奏中止();
				}
                else if ( keyboard.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_1)) 
                {
                ...
                else if (keyboard.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_2)) 
                {
                    if (!TJAPlayer3.DTX.bHasBranch[TJAPlayer3.stage選曲.n確定された曲の難易度]) return;
                ...
                else if (keyboard.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_3)) 
                {
                    if (!TJAPlayer3.DTX.bHasBranch[TJAPlayer3.stage選曲.n確定された曲の難易度]) return;
                ...
                //else if (keyboard.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_D1))
                //{
                ...
                //}
                //else if (keyboard.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_D2))		// #24243 2011.1.16 yyagi UI for InputAdjustTime in playing screen.
                //{
                //    if (this.n分岐した回数[1] < CDTXMania.DTX.listBRANCH.Count)
                ...
                //    this.b強制的に分岐させた = true;
                //}
                //else if (keyboard.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_D3))		// #24243 2011.1.16 yyagi UI for InputAdjustTime in playing screen.
                //{
                //    if (this.n分岐した回数[1] < CDTXMania.DTX.listBRANCH.Count)
				...
				if ( keyboard.IsPushedKey( (int)DxLibDLL.DX.KEY_INPUT_F4))
                {
				...
				else if ( keyboard.IsPushedKey( (int)DxLibDLL.DX.KEY_INPUT_F5))
                {
...
#if DEBUG
                if ( keyboard.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_F6 ) )
                {
                ...
                if ( keyboard.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_F7 ) )
                {
                ...
                if (!pChip.bHit)
                {
                    if( pChip.nチャンネル番号 >= 0x11 && pChip.nチャンネル番号 <= 0x14 || pChip.nチャンネル番号 == 0x1A || pChip.nチャンネル番号 == 0x1B )//|| pChip.nチャンネル番号 == 0x9A )
                    {
                        if ( ( pChip.n発声時刻ms + 120 ) < n現在時刻ms )
                        {
                            ...
                            //( ( pChip.nバーからの距離dot.Taiko < -40 ) && ( this.e指定時刻からChipのJUDGEを返す( ct演奏用タイマー.db現在の値, pChip, nInputAdjustTime ) == E判定.Miss ) ) )
                            {
                ...
                if( cChipCurrentlyInProcess != null && !cChipCurrentlyInProcess.bHit )
                {
                    if( cChipCurrentlyInProcess.nチャンネル番号 >= 0x13 && cChipCurrentlyInProcess.nチャンネル番号 <= 0x15 )//|| pChip.nチャンネル番号 == 0x9A )
                    {
				        if ( ( ( cChipCurrentlyInProcess.nバーからの距離dot.Taiko < -500 ) && ( cChipCurrentlyInProcess.n発声時刻ms <= n現在時刻ms && cChipCurrentlyInProcess.nノーツ終了時刻ms >= n現在時刻ms ) ) )
                           //( ( chip現在処理中の連打チップ.nバーからのノーツ末端距離dot.Taiko < -500 ) && ( chip現在処理中の連打チップ.n発声時刻ms <= ct演奏用タイマー.db現在の値 && chip現在処理中の連打チップ.nノーツ終了時刻ms >= ct演奏用タイマー.db現在の値 ) ) )
                           //( ( pChip.n発声時刻ms <= ct演奏用タイマー.db現在の値 && pChip.nノーツ終了時刻ms >= ct演奏用タイマー.db現在の値 ) ) )
		    		    {
                            if( bAutoPlay )
...
#region [ 01: BGM ]
					case 0x01:	// BGM
						if ( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
						{
							...
							{
								dTX.tチップの再生( pChip, pChip.n発声時刻ms, (int) Eレーン.BGM );
							}
						}
						break;
#endregion
					...
					case 0x50:	// 小節線
						{
                            if ( !pChip.bHit && ( pChip.nバーからの距離dot.Taiko < 0 ) )
						    {
                                ...
                                if( this.actPlayInfo.NowMeasure[nPlayer] == 0 )
                                {
                                    for (int i = 0; i < 2; i++)
                                    {
                                        ctChipAnime[i] = new CCounter(0, 3, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * 1 / 4, null);
                                    }
                                    ...
                                    if (TJAPlayer3.Skin.Game_Chara_Ptn_Normal != 0)
                                    {
                                        ...
                                        this.actChara.ctChara_Normal = new CCounter(0, this.actChara.arモーション番号.Length - 1, dbPtn_Normal, null);
                                    }
                                    else
                                    {
                                    ...
                                    if (TJAPlayer3.Skin.Game_Chara_Ptn_Clear != 0)
                                    {
                                        ...
                                        this.actChara.ctChara_Clear = new CCounter(0, this.actChara.arクリアモーション番号.Length - 1, dbPtn_Clear, null);
                                    }
                                    else
                                    {
                                    ...
                                    if (TJAPlayer3.Skin.Game_Chara_Ptn_GoGo != 0)
                                    {
                                        ...
                                        this.actChara.ctChara_GoGo = new CCounter(0, this.actChara.arゴーゴーモーション番号.Length - 1, dbPtn_GoGo, null);
                                    }
                                    else
                                    {
                                        this.actChara.ctChara_GoGo = new CCounter();
                                    }
                                    this.ct制御タイマ = new CCounter(0, 10, 500, null);
                                    if (TJAPlayer3.Skin.Game_Dancer_Ptn != 0)
                                    {
                                        ...
                                        this.actDancer.ct踊り子モーション = new CCounter(0, this.actDancer.ar踊り子モーション番号.Length - 1, dbUnit_dancer * TJAPlayer3.Skin.Game_Dancer_Beat, null);
                                    }
                                    else
                                    {
                                    ...
                                    {
                                        this.actMob.ctMob = new CCounter(1, 180, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * TJAPlayer3.Skin.Game_Mob_Beat / 180, null);
                                        this.actMob.ctMobPtn = new CCounter(0, TJAPlayer3.Skin.Game_Mob_Ptn - 1, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * TJAPlayer3.Skin.Game_Mob_Ptn_Beat / TJAPlayer3.Skin.Game_Mob_Ptn, null);
                                    }
                                    else
                                    {
                                    ...
                                    //for (int i = 0; i < 2; i++)
                                    //{
                                    //    ctChipAnime[i] = new CCounter(0, 3, 60.0 / CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM * 1 / 4, null);
                                    //}

                                    ////this.actChara.ctChara_Normal.db現在の値 = 0;

                                    ////this.actDancer.ct通常モーション = new CCounter( 0, this.actDancer.arモーション番号_通常.Length - 1, ( dbUnit * 4.0) / this.actDancer.arモーション番号_通常.Length, null );

                                    //// ↓踊り子・モブ↓
                                    //this.actDancer.ct踊り子モーション = new CCounter(0, this.actDancer.ar踊り子モーション番号.Length - 1, (dbUnit * CDTXMania.Skin.Game_Dancer_Beat) / this.actDancer.ar踊り子モーション番号.Length, null);
                                    ////this.actChara.ctモブモーション = new CCounter(0, this.actChara.arモブモーション番号.Length - 1, (dbUnit) / this.actChara.arモブモーション番号.Length, null);
                                    //this.actDancer.ct踊り子モーション.db現在の値 = 0;
                                    ////this.actChara.ctモブモーション.db現在の値 = 0;
                                    //this.actMob.ctMob = new CCounter(1, 180, 60.0 / CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM * CDTXMania.Skin.Game_Mob_Beat / 180, null);
                                    //this.actMob.ctMobPtn = new CCounter(0, CDTXMania.Skin.Game_Mob_Ptn - 1, 60.0 / CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM * CDTXMania.Skin.Game_Mob_Ptn_Beat / CDTXMania.Skin.Game_Mob_Ptn, null);
                                    //// ↑踊り子・モブ↑
                                    ////this.actDancer.ctモブ = new CCounter( 1.0, 16.0, (( dbUnit / 16.0 )), null );
						...
						if ( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
						{
                            ...
                            if( pChip.nコース == this.n現在のコース[ nPlayer ] )
                            {
                                 ...
                                 * for( int dancer = 0; dancer < 5; dancer++ )
                                    this.actDancer.st投げ上げ[ dancer ].ct進行 = new CCounter( 0, this.actDancer.arモーション番号_登場.Length - 1, dbUnit / this.actDancer.arモーション番号_登場.Length, null );

                                this.actDancer.ct通常モーション = new CCounter( 0, this.actDancer.arモーション番号_通常.Length - 1, ( dbUnit * 4 ) / this.actDancer.arモーション番号_通常.Length, null );
                                this.actDancer.ctモブ = new CCounter( 1.0, 16.0, (int)((60.0 / bpm / 16.0 ) * 1000 ), null );
//#endif
                               */
                            }
                            ...
                            if(nPlayer == 0)
                            {
                                if (TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.b終了値に達した)
                                {
                                    if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] < 100)
                                    {
                                        ...
                                        // ゴーゴースタート_ノーマル
                                        this.actChara.アクションタイマーリセット();
                                        this.actChara.ctキャラクターアクション_ゴーゴースタート = new CCounter(0, TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart) * 2, null);
                                ...
                                if (TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart_Max != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.b終了値に達した)
                                {
                                    if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] >= 100)
                                    {
                                        ...
                                        // ゴーゴースタート_MAX
                                        this.actChara.アクションタイマーリセット();
                                        this.actChara.ctキャラクターアクション_ゴーゴースタートMAX = new CCounter(0, TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart_Max - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart_Max) * 2, null);
                    ...
                    case 0xDE: //譜面分岐スタート
                        ...
                        if( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
                        {
                            ...
                            if( !this.bLEVELHOLD[ nPlayer ] )
                            {
                                ...
                                this.t分岐処理(this.n次回のコース[nPlayer], pChip.n分岐発生時刻ms, nPlayer);
...
#region[ 15-19: 連打 ]
        ...
        public void t分岐処理(int n分岐先, long n分岐時刻Ms, int nPlayer)
        {
            ...
            {
                var A = i;
                if ((dTX.listChip[A].nチャンネル番号 >= 0x11 && dTX.listChip[A].nチャンネル番号 <= 0x19) || dTX.listChip[A].nチャンネル番号 == 0x50)
                {
                    if (dTX.listChip[A].n発声時刻ms >= n分岐時刻Ms)
                    {
        ...
        protected double GetNowPBMTime( CDTX tja, double play_time )
        {
            double bpm_time = 0;
            int last_input = 0;
            double last_bpm_change_time;
            play_time = ct演奏用タイマー.db現在の値 - tja.nOFFSET;
			...
			for ( int i = this.n現在のトップChip; i >= 0; i-- )
			{
				...
				if ( ( pChip.n発声時刻ms + nDuration > 0 ) && ( pChip.n発声時刻ms <= nStartTime ) && ( nStartTime <= pChip.n発声時刻ms + nDuration ) )
				{
					if ( pChip.bWAVを使うチャンネルである && ( pChip.nチャンネル番号 >> 4 ) != 0xB )	// wav系チャンネル、且つ、空打ちチップではない
					{
						...
						{
							TJAPlayer3.DTX.tチップの再生( pChip, pChip.n発声時刻ms, (int) Eレーン.BGM );
#region [ PAUSEする ]
							int j = wc.n現在再生中のサウンド番号;
							if ( wc.rSound[ j ] != null )
							{
							    wc.rSound[ j ].Stop();
							    wc.rSound[ j ].t再生を開始する( nStartTime - pChip.n発声時刻ms );
...
#region [ タイマを再開して、PAUSEから復帰する ]
			ct演奏用タイマー.db現在の値 = nStartTime;
            this.ct演奏用タイマー.t開始(nStartTime, (double)int.MaxValue, 0.001, null);
			//CDTXMania.Timer.t再開();
		...
		{
			if (TJAPlayer3.Tx.Background != null )
			{
				TJAPlayer3.Tx.Background.t2D描画( 0, 0 );
			}
		}
		...
		protected void t進行描画_判定ライン()
		{
			if ( TJAPlayer3.ConfigIni.eDark != Eダークモード.FULL )
			{
				...
				{
				    this.txヒットバー.t2D描画( 295, y, new Rectangle( 0, 0, 558, 6 ) );
				}
			}
		}
