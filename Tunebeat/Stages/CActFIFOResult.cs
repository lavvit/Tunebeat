		...
		public override int On進行描画()
		{
			...
			if (TJAPlayer3.Tx.Result_FadeIn != null)
			{
                if( this.mode == EFIFOモード.フェードアウト )
                {
                    ...
                    TJAPlayer3.Tx.Result_FadeIn.t2D描画( 0, this.counter.n現在の値 >= 360 ? 0 : -360 + y, new Rectangle( 0, 0, 1280, 380 ) );
                    TJAPlayer3.Tx.Result_FadeIn.t2D描画( 0, 720 - y, new Rectangle( 0, 380, 1280, 360 ) );
                }
                else
                {
                    TJAPlayer3.Tx.Result_FadeIn.Opacity = (((100 - this.counter.n現在の値) * 0xff) / 100);
                    TJAPlayer3.Tx.Result_FadeIn.t2D描画( 0, 0, new Rectangle( 0, 0, 1280, 360 ) );
                    TJAPlayer3.Tx.Result_FadeIn.t2D描画( 0, 360, new Rectangle( 0, 380, 1280, 360 ) );
                }
