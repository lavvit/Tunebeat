        ...
        public override int On進行描画()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                if (this.ct上背景スクロール用タイマー[i] != null)
                {
                    ...
                    //int nループ幅 = 328;
                    TJAPlayer3.Tx.Background_Up[i].t2D描画(0 - this.ct上背景スクロール用タイマー[i].n現在の値, TJAPlayer3.Skin.Background_Scroll_Y[i]);
                    for (int l = 1; l < ForLoop + 1; l++)
                    {
                        TJAPlayer3.Tx.Background_Up[i].t2D描画(+(l * TJAPlayer3.Tx.Background_Up[i].szテクスチャサイズ.Width) - this.ct上背景スクロール用タイマー[i].n現在の値, TJAPlayer3.Skin.Background_Scroll_Y[i]);
                    }
                }
                if (this.ct上背景スクロール用タイマー[i] != null)
                {
                    ...
                    TJAPlayer3.Tx.Background_Up_Clear[i].t2D描画(0 - this.ct上背景スクロール用タイマー[i].n現在の値, TJAPlayer3.Skin.Background_Scroll_Y[i]);
                    for (int l = 1; l < ForLoop + 1; l++)
                    {
                        TJAPlayer3.Tx.Background_Up_Clear[i].t2D描画((l * TJAPlayer3.Tx.Background_Up_Clear[i].szテクスチャサイズ.Width) - this.ct上背景スクロール用タイマー[i].n現在の値, TJAPlayer3.Skin.Background_Scroll_Y[i]);
                    }
                }
            ...
            {
                {
                    ...
                    {
                        TJAPlayer3.Tx.Background_Down.t2D描画( 0, 360 );
                    }
                }
                if(TJAPlayer3.stage演奏ドラム画面.bIsAlreadyCleared[0])
                {
                    if( TJAPlayer3.Tx.Background_Down_Clear != null && TJAPlayer3.Tx.Background_Down_Scroll != null )
                    {
                        ...
                        TJAPlayer3.Tx.Background_Down_Clear.t2D描画( 0, 360 );
                        ...
                        //int nループ幅 = 328;
                        TJAPlayer3.Tx.Background_Down_Scroll.t2D描画(0 - this.ct下背景スクロール用タイマー1.n現在の値, 360);
                        for (int l = 1; l < ForLoop + 1; l++)
                        {
                            TJAPlayer3.Tx.Background_Down_Scroll.t2D描画(+(l * TJAPlayer3.Tx.Background_Down_Scroll.szテクスチャサイズ.Width) - this.ct下背景スクロール用タイマー1.n現在の値, 360);
                        }
