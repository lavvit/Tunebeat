...
namespace TJAPlayer3
{
    ...
    protected float[,] nコンボ拡大率_座標_1000combo = new float[,]{
      	...
      	protected virtual void tコンボ表示_太鼓( int nCombo値, int nジャンプインデックス, int nPlayer )
		{
            ...
            if( n桁数 <= 2 )
            {
                TJAPlayer3.Tx.Taiko_Combo_Text?.t2D拡大率考慮下中心基準描画(TJAPlayer3.Skin.Game_Taiko_Combo_Text_X[nPlayer], TJAPlayer3.Skin.Game_Taiko_Combo_Text_Y[nPlayer], new Rectangle(0, 0, TJAPlayer3.Skin.Game_Taiko_Combo_Text_Size[0], TJAPlayer3.Skin.Game_Taiko_Combo_Text_Size[1]));
            }
            else
            {
                TJAPlayer3.Tx.Taiko_Combo_Text?.t2D拡大率考慮下中心基準描画(TJAPlayer3.Skin.Game_Taiko_Combo_Text_X[nPlayer], TJAPlayer3.Skin.Game_Taiko_Combo_Text_Y[nPlayer], new Rectangle(0, TJAPlayer3.Skin.Game_Taiko_Combo_Text_Size[1], TJAPlayer3.Skin.Game_Taiko_Combo_Text_Size[0], TJAPlayer3.Skin.Game_Taiko_Combo_Text_Size[1]));
            }
            #endregion
            ...
            for ( int i = 0; i < n桁数; i++ )
            {
                ...
                if ( n桁数 <= 1 )
                {
				    if(TJAPlayer3.Tx.Taiko_Combo[0] != null )
				    {
                        ...
                        TJAPlayer3.Tx.Taiko_Combo[0].t2D拡大率考慮下中心基準描画( rightX, TJAPlayer3.Skin.Game_Taiko_Combo_Y[nPlayer] , new Rectangle( n位の数[ i ] * TJAPlayer3.Skin.Game_Taiko_Combo_Size[0], 0, TJAPlayer3.Skin.Game_Taiko_Combo_Size[0], TJAPlayer3.Skin.Game_Taiko_Combo_Size[1]) );
				    }
                }
                else if( n桁数 <= 2 )
                {
                    ...
                    if (TJAPlayer3.Tx.Taiko_Combo[0] != null )
				    {
                        ...
                        TJAPlayer3.Tx.Taiko_Combo[0].vc拡大縮小倍率.Y = TJAPlayer3.Skin.Game_Taiko_Combo_Scale[0] + yScalling;
                        TJAPlayer3.Tx.Taiko_Combo[0].vc拡大縮小倍率.X = TJAPlayer3.Skin.Game_Taiko_Combo_Scale[0];
                        TJAPlayer3.Tx.Taiko_Combo[0].t2D拡大率考慮下中心基準描画( rightX - TJAPlayer3.Skin.Game_Taiko_Combo_Padding[0] * i, TJAPlayer3.Skin.Game_Taiko_Combo_Y[nPlayer], new Rectangle( n位の数[ i ] * TJAPlayer3.Skin.Game_Taiko_Combo_Size[0], 0, TJAPlayer3.Skin.Game_Taiko_Combo_Size[0], TJAPlayer3.Skin.Game_Taiko_Combo_Size[1]) );
				    }
                }
                else if( n桁数 == 3 )
                {
                    ...
                    if(TJAPlayer3.Tx.Taiko_Combo_Effect != null )
                    {
                        ...
                        if (ctコンボラメ.n現在の値 > 14)
                        {
                            ...
                            #endregion
                            TJAPlayer3.Tx.Taiko_Combo_Effect.t2D拡大率考慮下中心基準描画((rightX - TJAPlayer3.Skin.Game_Taiko_Combo_Padding[1] * i), TJAPlayer3.Skin.Game_Taiko_Combo_Ex_Y[nPlayer] - ((TJAPlayer3.Skin.Game_Taiko_Combo_Size_Ex[1] / 4) * TJAPlayer3.Skin.Game_Taiko_Combo_Scale[1]) - (int)(1.05 * this.ctコンボラメ.n現在の値));
                        }
                    }
                }
                else
                {
                    ...
                    if (TJAPlayer3.Tx.Taiko_Combo_Effect != null)
                    {
                        ...
                        if (ctコンボラメ.n現在の値 > 14)
                        {
                            ...
                            #endregion
                            TJAPlayer3.Tx.Taiko_Combo_Effect.t2D拡大率考慮下中心基準描画((rightX - TJAPlayer3.Skin.Game_Taiko_Combo_Padding[1] * i), TJAPlayer3.Skin.Game_Taiko_Combo_Ex4_Y[nPlayer] - ((TJAPlayer3.Skin.Game_Taiko_Combo_Size_Ex[1] / 4) * TJAPlayer3.Skin.Game_Taiko_Combo_Scale[2]) - (int)(1.05 * this.ctコンボラメ.n現在の値));
		...
		public override int On進行描画()
		{
			...
			for( int i = 0; i < 4; i++ )
			{
				...
				{
					if( ( this.status[ i ].n前回の時刻_ジャンプ用 == -1 ) || (DxLibDLL.DX.GetNowCount() < this.status[ i ].n前回の時刻_ジャンプ用 ) )
						this.status[ i ].n前回の時刻_ジャンプ用 = DxLibDLL.DX.GetNowCount();

					const long INTERVAL = 2;
					while( (DxLibDLL.DX.GetNowCount() - this.status[ i ].n前回の時刻_ジャンプ用 ) >= INTERVAL )
					{
				...
				switch( this.status[ i ].e現在のモード )
				{
					...
					case EMode.進行表示中:
						...
						if( ( e今回の状態遷移イベント == EEvent.非表示 ) || ( e今回の状態遷移イベント == EEvent.ミス通知 ) )
						{
							...
							this.status[ i ].nコンボが切れた時刻 = DxLibDLL.DX.GetNowCount();
							goto Retry;
						}
					...
					case EMode.残像表示中:
						...
						if (( DxLibDLL.DX.GetNowCount() - this.status[ i ].nコンボが切れた時刻 ) > 1000 )
						{
							// モード変更２
							this.status[ i ].e現在のモード = EMode.非表示中;
							goto Retry;
