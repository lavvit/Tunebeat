		...
		public override int On進行描画()
		{
			...
			if( base.b初めての進行描画 )
			{
				...
				if( this.sd読み込み音 != null )
				{
					...
					this.nBGM再生開始時刻 =0;
					this.nBGMの総再生時間ms = DX.GetSoundTotalTime(this.sd読み込み音.Handle);
				}
				else
				{
            ...
            if (TJAPlayer3.Tx.SongLoading_Plate != null)
            {
                ...
                {
                TJAPlayer3.Tx.SongLoading_Plate.t2D描画(TJAPlayer3.Skin.SongLoading_Plate_X, TJAPlayer3.Skin.SongLoading_Plate_Y - (TJAPlayer3.Tx.SongLoading_Plate.szテクスチャサイズ.Height / 2));
                }
                else if(TJAPlayer3.Skin.SongLoading_Plate_ReferencePoint == CSkin.ReferencePoint.Right)
                {
                TJAPlayer3.Tx.SongLoading_Plate.t2D描画(TJAPlayer3.Skin.SongLoading_Plate_X - TJAPlayer3.Tx.SongLoading_Plate.szテクスチャサイズ.Width, TJAPlayer3.Skin.SongLoading_Plate_Y - (TJAPlayer3.Tx.SongLoading_Plate.szテクスチャサイズ.Height / 2));
                }
                else
                {
                TJAPlayer3.Tx.SongLoading_Plate.t2D描画(TJAPlayer3.Skin.SongLoading_Plate_X - (TJAPlayer3.Tx.SongLoading_Plate.szテクスチャサイズ.Width / 2), TJAPlayer3.Skin.SongLoading_Plate_Y - (TJAPlayer3.Tx.SongLoading_Plate.szテクスチャサイズ.Height / 2));
                }
            }
			...
			if( this.txタイトル != null )
			{
                ...
                {
                    this.txタイトル.t2D描画(TJAPlayer3.Skin.SongLoading_Title_X, TJAPlayer3.Skin.SongLoading_Title_Y - (this.txタイトル.szテクスチャサイズ.Height / 2) + nサブタイトル補正);
                }
                else if(TJAPlayer3.Skin.SongLoading_Title_ReferencePoint == CSkin.ReferencePoint.Right)
                {
                    this.txタイトル.t2D描画((int)(TJAPlayer3.Skin.SongLoading_Title_X - (this.txタイトル.szテクスチャサイズ.Width * txタイトル.vc拡大縮小倍率.X)), TJAPlayer3.Skin.SongLoading_Title_Y - (this.txタイトル.szテクスチャサイズ.Height / 2) + nサブタイトル補正);
                }
                else
                {
                    this.txタイトル.t2D描画((int)(TJAPlayer3.Skin.SongLoading_Title_X - ((this.txタイトル.szテクスチャサイズ.Width * txタイトル.vc拡大縮小倍率.X) / 2)), TJAPlayer3.Skin.SongLoading_Title_Y - (this.txタイトル.szテクスチャサイズ.Height / 2) + nサブタイトル補正);
                }
            }
			if( this.txサブタイトル != null )
			{
                ...
                {
                    this.txサブタイトル.t2D描画(TJAPlayer3.Skin.SongLoading_SubTitle_X, TJAPlayer3.Skin.SongLoading_SubTitle_Y - (this.txサブタイトル.szテクスチャサイズ.Height / 2));
                }
                else if(TJAPlayer3.Skin.SongLoading_Title_ReferencePoint == CSkin.ReferencePoint.Right)
                {
                    this.txサブタイトル.t2D描画((int)(TJAPlayer3.Skin.SongLoading_SubTitle_X - (this.txサブタイトル.szテクスチャサイズ.Width * txタイトル.vc拡大縮小倍率.X)), TJAPlayer3.Skin.SongLoading_SubTitle_Y - (this.txサブタイトル.szテクスチャサイズ.Height / 2));
                }
                else
                {
                    this.txサブタイトル.t2D描画((int)(TJAPlayer3.Skin.SongLoading_SubTitle_X - ((this.txサブタイトル.szテクスチャサイズ.Width * txサブタイトル.vc拡大縮小倍率.X) / 2)), TJAPlayer3.Skin.SongLoading_SubTitle_Y - (this.txサブタイトル.szテクスチャサイズ.Height / 2));
                }
            }
			//-----------------------------
			#endregion
			...
			switch( base.eフェーズID )
			{
				...
				case CStage.Eフェーズ.NOWLOADING_システムサウンドBGMの完了を待つ:
					{
						long nCurrentTime = DX.GetNowCount();
...
//						if ( ( nCurrentTime - this.nBGM再生開始時刻 ) > ( this.nBGMの総再生時間ms - 1000 ) )
		...
		{
			if 	( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_ESCAPE ) )		// escape (exit)
			{
				return true;
			}
			return false;
		...
		private void ShowProgressByFilename(string strファイル名 )
		{
			if ( graphicsFilename != null && ftFilename != null )
			{
				...
				txFilename = new CTexture( bitmapFilename );
				txFilename.vc拡大縮小倍率 = new PointF( 0.5f, 0.5f );
				txFilename.t2D描画( 0, 720 - 16 );
			}
		}
