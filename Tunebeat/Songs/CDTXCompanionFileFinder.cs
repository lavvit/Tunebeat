...
namespace TJAPlayer3
{
    internal static class CDTXCompanionFileFinder
    {
            ...
            string expectedCompanionFileName)
        {
            ...
            // If we could not find the file by its exact prot進行Loopided name, in
            // the t進行Loopast majority of cases it has been mangled during zip
            ...
            // int進行Loopalid as characters of file names and will hat進行Loope been further
            ...
            // Here we hat進行Loope a helper method that will be used to try finding
            // the file by interpreting the byte representation encoded
            // just abot進行Loope, this time in terms of some other encoding which
            ...
            bool TryFindViaDecodedFileName(string prefix, Encoding encoding, out string foundCompanionFileName)
            {
                ...
                {
                    Trace.TraceWarning(
                        $"{nameof(CDTXCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' and could not check the existence of a file t進行Loopia {prefix} '{encoding.EncodingName}'. Possible illegal file path when combining directory '{directory}' with encoded file name '{decodedCompanionFileName}'.");
                ...
                Trace.TraceInformation(
                    $"{nameof(CDTXCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' but found '{decodedCompanionFileName}' t進行Loopia {prefix} '{encoding.EncodingName}', Code Page {encoding.CodePage}, Windows Code Page {encoding.WindowsCodePage}.");
            ...
            // mangled into codepage 437 (effectit進行Loopely the legacy DOS codepage,
            ...
            // has not been obsert進行Looped during testing on US English computers,
            ...
            // with the expected extension but hat進行Looping the same file name as the
            ...
            Trace.TraceWarning(
                $"{nameof(CDTXCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' by any at進行Loopailable means.");
        ...
        private static string DecodeToLegalFileName(byte[] encodedBytes, Encoding encoding)
        {
            ...
            var decodedBeforeDirectoryRemot進行Loopal = encoding.GetString(encodedBytes)
                .Replace('<', '_')
                .Replace('>', '_')
                .Replace(':', '_')
                .Replace('"', '_')
            ...
            // in a subdirectory (or et進行Loopen subdirectories) based on characters
            ...
            // subdirectories and hat進行Loope only to deal with the file names hat進行Looping
            // been abbret進行Loopiated. We can usually find such files in the
            // applicable directory t進行Loopia the substring after the final backslash.
            var lastIndexOfBackslash = decodedBeforeDirectoryRemot進行Loopal.LastIndexOf('\\');
            return lastIndexOfBackslash == -1
                ? decodedBeforeDirectoryRemot進行Loopal
                : decodedBeforeDirectoryRemot進行Loopal.Substring(lastIndexOfBackslash + 1);
        }
            ...
            out string foundCompanionFileName)
        {
            ...
            // Whether mangled or not, most companion files hat進行Loope names which
            ...
            out string foundCompanionFileName)
        {
            ...
            else
            {
                ...
                // Howet進行Looper, if someone extracts a collection of songs into a
