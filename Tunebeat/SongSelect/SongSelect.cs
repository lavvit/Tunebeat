...
namespace Tunebeat
{
    public class SongSelect : Scene
    {
        ...
        {
            SongLoad.Init();
            if (SongData.Song.Count > 0) NowTJA = SongData.Song[NowSongNumber];
            Title = new Texture();
        ...
        public override void Draw()
        {
            ...
            if (NowTJA != null)
            {
                ...
                switch (NowTJA.Type)
                {
                    case EType.Score:
                        ...
                        for (int i = 0; i < 5; i++)
                        {
                            if (i == (select && !Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 0)) || (PlayData.Data.IsPlay2P && i == (select && Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 1))) || PlayData.Data.PreviewType == 3)
                            {
                                switch (i)
                                {
                        ...
                        if (NowTJA.Course[select && !Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 0)].LEVEL > 0)
                        {
                            int lev = NowTJA.Course[select && !Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 0)].LEVEL < 12 ? NowTJA.Course[select && !Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 0)].LEVEL : 12;
                            for (int i = 0; i < lev; i++)
                            {
                                switch (select && !Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 0))
                                {
                                    case (int)ECourse.Easy:
                                ...
                                if (NowTJA.Course[EnableCourse(NowTJA.Course, 0)].LEVEL > 12)
                                {
                                    ...
                                    for (int j = 0; j < NowTJA.Course[select && !Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 0)].LEVEL - 12; j++)
                                    {
                                        switch (select && !Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 0))
                                        {
                                            case (int)ECourse.Easy:
                        ...
                        if (PlayData.Data.IsPlay2P && NowTJA.Course[select && Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 1)].LEVEL > 0)
                        {
                            int lev = NowTJA.Course[select && Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 1)].LEVEL < 12 ? NowTJA.Course[select && Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 1)].LEVEL : 12;
                            for (int i = 0; i < lev; i++)
                            {
                                switch (select && Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 1))
                                {
                                    case (int)ECourse.Easy:
                                        TextureLoad.SongSelect_Difficulty_Level.Color = Color.FromArgb(0xff5f3f);
                                        break;
                                ...
                                if (NowTJA.Course[select && Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 1)].LEVEL > 12)
                                {

                                    for (int j = 0; j < NowTJA.Course[select && Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 1)].LEVEL - 12; j++)
                                    {
                                        switch (select && Key.IsPushing(EKey.LShift) ? wcursor : EnableCourse(NowTJA.Course, 1))
                                        {
                                            case (int)ECourse.Easy:
            ...
            DrawString(200, 20, $"SongCount:{SongData.Song.Count},{SongData.AllSong.Count}", 0xffffff);
        ...
        public override void Update()
        {
            if (Preview != null && Preview.IsEnable && !Preview.IsPlaying)
            {
                ...
                Preview.PlayLoop(new TJAParse.TJAParse(NowTJA.Path).Header.DEMOSTART, true);
            ...
            else
            {
                ...
                {
                    NowTJA = SongData.Song[0];
                    Back();
                }

                if ((Key.IsPushed(EKey.Enter) || Key.IsPushed(PlayData.Data.LEFTDON) || Key.IsPushed(PlayData.Data.RIGHTDON)) && NowTJA != null)
                {
                    SoundLoad.Don[0].Play();
                    Enter();
                }
                ...
                for (int i = 0; i < 5; i++)
                {
                    if (CourseBar[i].LPushed)
                    {
                        ...
                        SetScore();
                        string path = NowTJA.Path;
                        SongLoad.Sort(SongData.Song, SongLoad.NowSort);
                        SongLoad.Sort(SongData.AllSong, SongLoad.NowSort);

                        for (int j = 0; j < SongData.Song.Count; j++)
                        {
                            if (SongData.Song[j].Path == path)
                            {
                                NowSongNumber = j;
                                break;
                            }
                        }
                        NowTJA = SongData.Song[NowSongNumber];
                    }
                }
                ...
                if (Key.IsPushed(EKey.Space))
                {
                    ...
                    {
                        if (SongData.Song[i].Title == title)
                        {
                            NowSongNumber = i;
                            break;
                        }
        ...
        {
            NowTJA = NowTJA.Next;
            if (NowSongNumber >= SongData.Song.Count - 1) NowSongNumber = 0;
            else NowSongNumber++;
        }
        public static void Prev()
        {
            NowTJA = NowTJA.Prev;
            if (NowSongNumber <= 0) NowSongNumber = SongData.Song.Count - 1;
            else NowSongNumber--;
        }
            ...
            switch (NowTJA.Type)
            {
                case EType.Score:
                    if (File.Exists(NowTJA.Path))
                    {
                        if (NowTJA.Course[EnableCourse(NowTJA.Course, 0)].IsEnable || PlayData.Data.PreviewType == 3)
                        {
                            ...
                            for (int i = 0; i < SongData.Song.Count; i++)
                            {
                                if (SongData.Song[i].Type != EType.Score)
                                    NowSListNumber--;
                            }
                            if (Preview != null) { Preview.Stop(); Preview = null; }
                            Program.SceneChange(new Game());
                ...
                case EType.Random:
                    while (true)
                    {
                        ...
                        int r = random.Next(SongData.AllSong.Count);
                        if (PlayData.Data.PlayCourse[0] == (int)ECourse.Edit)
                        {
                            Random difran = new Random();
                            int d = difran.Next(0, 3);
                            Course = d > 0 ? 4 : 3;
                            if (SongData.AllSong[r] != null && SongData.AllSong[r].Course[Course].IsEnable)
                            {
                                NowTJA = SongData.AllSong[r];
                                break;
                            }
                        }
                        else
                        {
                            if (SongData.AllSong[r] != null && SongData.AllSong[r].Course[PlayData.Data.PlayCourse[0]].IsEnable)
                            {
                                NowTJA = SongData.AllSong[r];
                                break;
                            }
                ...
                case EType.Folder:
                    ...
                    NowPath = NowTJA.Path;
                    //SongLoad.Load(SongData.Song, NowTJA.Path);
                    NowSongNumber = 0;
                    NowTJA = SongData.Song[NowSongNumber];
                    if (PlayData.Data.FontRendering) FontLoad();
                    Reload();
        ...
        public static void FontLoad()
        {
            ...
            {
                Title = FontRender.GetTexture(NowTJA.Header.TITLE, 96, 6, PlayData.Data.FontName);
            }
            else Title = new Texture();
            if (!string.IsNullOrEmpty(NowTJA.Header.GENRE))
            {
                Genre = FontRender.GetTexture(NowTJA.Header.GENRE, 32, 4, PlayData.Data.FontName);
            }
            else Genre = new Texture();
            if (!string.IsNullOrEmpty(NowTJA.Header.SUBTITLE))
            {
                SubTitle = FontRender.GetTexture(NowTJA.Header.SUBTITLE, 32, 4, PlayData.Data.FontName);
            }
            else SubTitle = new Texture();
            BPM = FontRender.GetTexture($"{Math.Round(NowTJA.Header.BPM, 0, MidpointRounding.AwayFromZero)} BPM", 32, 6, PlayData.Data.FontName);
        }
