...
namespace Tunebeat.Common
{
    ...
    {
        public static Texture GetTexture(string text, FontFamily fontName, int size, int edge, int interval, Color foreColor, Color backColor, int space = 16)
        {
            if(text == null || text == "") { return new Texture(); }
            if(space < 1) space = 16;
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                // パスに文字を追加
                gp.AddString(text[i].ToString(), fontName, 0, size, new Point(8, 8), stringFormat);
                // 縁取りをする。
