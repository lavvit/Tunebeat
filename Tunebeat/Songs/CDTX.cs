...
namespace TJAPlayer3
{
    internal class CDTX : CActivity
    {
        ...
        public bool[] b最初の分岐小節 = new bool[5];
        // クラス
        public class CBPM
        {
        ...
        public class CChip : IComparable<CDTX.CChip>, ICloneable
        {
            ...
            public int GetDuration()
            {
                ...
                if (this.bWAVを使うチャンネルである)       // WAV
                {
                    ...
                    {
                        nDuration = (wc.rSound[0] == null) ? 0 : (int)DxLibDLL.DX.GetSoundTotalTime(wc.rSound[0].Handle);
                    }
                }
        ...
        public class CWAV : IDisposable
        {
            ...
            public int n位置;
            public long[] n一時停止時刻 = new long[TJAPlayer3.ConfigIni.nPoliphonicSounds];   
        ...
        public CChip NowBranchChip = null;
        ...
        private int[] nNowRoll = new int[5];
        private int[] nNowRollCount = new int[5];
...
//		public bool bBASS演奏で直前のBASSを消音する;
        ...
        public void tWave再生位置自動補正(double time)
        {
            foreach (CWAV cwav in this.listWAV.Values)
            {
                this.tWave再生位置自動補正(cwav, time);
            }
        }
        public void tWave再生位置自動補正(CWAV wc, double time)
        {
            if (wc.rSound[0] != null && DxLibDLL.DX.GetSoundTotalTime(wc.rSound[0].Handle) >= 5000)
            {
                for (int i = 0; i < nPolyphonicSounds; i++)
                {
                    if ((wc.rSound[i] != null) && (wc.rSound[i].IsPlaying))
                    {
                        wc.rSound[i].t再生を開始する((long)time);
                    }
                }
            }
        }
        ...
        public void tWAVの読み込み(CWAV cwav)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < cwav.rSound.Length; i++)
                {
                    try
                    {
                        cwav.rSound[i] = new CSound(str);
            ...
            if (pChip.n整数値_内部番号 >= 0)
            {
                ...
                if (this.listWAV.TryGetValue(pChip.n整数値_内部番号, out CWAV wc))
                {
                    ...
                    if (sound != null)
                    {
                        ...
                        sound.t再生を開始する(TJAPlayer3.DTX.nOFFSET >= 0.0 ? 0 : (int)(-TJAPlayer3.DTX.nOFFSET));
                    }
        ...
        public void t全チップの再生一時停止()
        {
            foreach (CWAV cwav in this.listWAV.Values)
            {
                for (int i = 0; i < nPolyphonicSounds; i++)
                {
                    ...
                    {
                        cwav.rSound[i].Stop();
                        cwav.n一時停止時刻[i] = DxLibDLL.DX.GetNowCount();
                    }
                }
            }
        }
            ...
            foreach (CWAV cwav in this.listWAV.Values)
            {
                for (int i = 0; i < nPolyphonicSounds; i++)
                {
                    if ((cwav.rSound[i] != null) && cwav.rSound[i].IsPlaying)
                    {
                        ...
                        cwav.rSound[i].t再生を開始する(cwav.n一時停止時刻[i] - cwav.n再生開始時刻[i]);
                        cwav.n再生開始時刻[i] += DxLibDLL.DX.GetNowCount() - cwav.n一時停止時刻[i];
                    }
                }
            }
        }
            ...
            if (!string.IsNullOrEmpty(str全入力文字列))
            {
                ...
                if (ce.MoveNext())
                {
                    ...
                    if (!this.bヘッダのみ)
                    {
                        ...
                        int nBar = 0;
                        int nCount = 0;
                        for (int i = 0; i < 5; i++) this.nNowRollCount[i] = 0;
                        ...
                        foreach (CChip chip in this.listChip)
                        {
                            ...
                            nCount++;
                            this.nNowRollCount[chip.nコース] = 0;
                                ...
                                case 0x17:
                                    {
                                        ...
                                        this.nNowRoll[chip.nコース] = this.nNowRollCount[chip.nコース] - 1;
                                        continue;
                                    }
                                case 0x18:
                                    {
        ...
        private void t命令を挿入する(string InputText)
        {
            ...
            else if (command == "#BRANCHSTART")
            {
                ...
                chip.nチャンネル番号 = 0xDE;
                chip.n発声位置 = ((this.n現在の小節数 - 1) * 384);
                chip.n発声時刻ms = (int)(this.dbNowTime - ((15000.0 / this.dbNowBPM * (this.fNow_Measure_s / this.fNow_Measure_m)) * (n条件 == 1 ? 14.0 : 16.0))); //ここの時間設定は前の小節の開始時刻である必要があるのだが...
                chip.n分岐発生時刻ms = (int)(this.dbNowTime); //ここの時間設定は前の小節の開始時刻である必要があるのだが...
                chip.dbSCROLL = this.dbNowScroll;
                chip.dbBPM = this.dbNowBPM;
                chip.n整数値_内部番号 = this.n内部番号BRANCH1to;

                this.NowBranchChip = chip;
                // チップを配置。
                this.listChip.Add(chip);

                for (int i = 0; i < b最初の分岐小節.Length; i++) b最初の分岐小節[i] = true;
                this.n内部番号BRANCH1to++;
            }
            else if (command == "#N" || command == "#E" || command == "#M")
            {
                switch (command)
                {
                    case "#N": this.n現在のコース = 0; break;//分岐:普通譜面
                    case "#E": this.n現在のコース = 1; break;//分岐:玄人譜面
                    case "#M": this.n現在のコース = 2; break;//分岐:達人譜面
                }
                if (!listBRANCH.TryGetValue(this.n内部番号BRANCH1to - 1, out var branch))
                {
                    Trace.TraceWarning($"正常ではない.tjaファイルを読み込みました。" + command + " 命令がありません。 ({strファイル名の絶対パス})");
                    return;
                }

                this.n現在の小節数 = (this.NowBranchChip.n発声位置 / 384) + 1;
                this.dbNowTime = this.NowBranchChip.n分岐発生時刻ms;
                this.dbNowBPM = this.NowBranchChip.dbBPM;
                this.dbNowScroll = this.NowBranchChip.dbSCROLL;
                this.dbNowBMScollTime = this.NowBranchChip.fBMSCROLLTime;
            }
                ...
                else
                {
                    if (this.b小節線を挿入している == false)
                    {
                        for (int i = 0; i < (IsEndedBranching == true ? 3 : 1); i++)
                        {
                            ...
                            if (b最初の分岐小節[chip.nコース])
                            {
                                chip.bBranch = true;
                                b最初の分岐小節[chip.nコース] = false;
                            }
                            this.listChip.Add(chip);
                        }
                        ...
                        if (nObjectNum != 0)
                        {
                            for (int i = 0; i < (IsEndedBranching == true ? 3 : 1); i++)
                            {
                                ...
                                if ((nObjectNum >= 5 && nObjectNum <= 7) || nObjectNum == 9)
                                {
                                    ...
                                    {
                                        this.nNowRollCount[chip.nコース] = listChip.Count;
                                        nNowRoll[chip.nコース] = nObjectNum;
                                    }
                                }
                                ...
                                if (nObjectNum == 8)
                                {
                                    ...
                                    chip.nノーツ出現時刻ms = listChip[nNowRollCount[chip.nコース]].nノーツ出現時刻ms;
                                    chip.nノーツ移動開始時刻ms = listChip[nNowRollCount[chip.nコース]].nノーツ移動開始時刻ms;

                                    chip.n連打音符State = nNowRoll[chip.nコース];
                                    listChip[nNowRollCount[chip.nコース]].nノーツ終了位置 = (this.n現在の小節数 * 384) + ((384 * n) / n文字数);
                                    listChip[nNowRollCount[chip.nコース]].nノーツ終了時刻ms = (int)this.dbNowTime;
                                    listChip[nNowRollCount[chip.nコース]].fBMSCROLLTime_end = (int)this.dbNowBMScollTime;
                                    //listChip[ nNowRollCount ].dbBPM = this.dbNowBPM;
                                    //listChip[ nNowRollCount ].dbSCROLL = this.dbNowSCROLL;
                                    nNowRoll[chip.nコース] = 0;
                                    //continue;
                                }
            ...
            foreach (CChip pChip in listChip)
            {
                switch (pChip.nチャンネル番号)
                {
                    ...
                    case 0x01:
                        ...
                        {
                            double _db再生速度 = (TJAPlayer3.DTXVmode.Enabled) ? this.dbDTXVPlaySpeed : this.db再生速度;
                            duration = (wc.rSound[0] == null) ? 0 : (int)(DxLibDLL.DX.GetSoundTotalTime(wc.rSound[0].Handle) / _db再生速度); // #23664 durationに再生速度が加味されておらず、低速再生でBGMが途切れる問題を修正 (発声時刻msは、DTX読み込み時に再生速度加味済)
                        }
