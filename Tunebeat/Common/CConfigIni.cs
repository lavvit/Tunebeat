			...
			get
			{
				for( int i = 0; i <= (int)EKeyConfigPart.SYSTEM; i++ )
				{
					for( int j = 0; j <= (int)EKeyConfigPad.Capture; j++ )
					{
						...
						{
							if( ( this.KeyAssign[ i ][ j ][ k ].入力デバイス == E入力デバイス.キーボード ) && ( this.KeyAssign[ i ][ j ][ k ].コード == (int) DxLibDLL.DX.KEY_INPUT_RETURN) )
							{
								return false;
							}
						}
...
//		public int nASIOBufferSizeMs;				// #24820 2012.12.28 yyagi ASIOのバッファサイズ
		...
		public CConfigIni()
		{
			...
			this.nウインドウwidth = 1280;			// #23510 2010.10.31 yyagi add
			...
			this.nSoundDeviceType = FDK.COS.bIsVistaOrLater ?
				(int) ESoundDeviceTypeForConfig.WASAPI_Kyoyu : (int) ESoundDeviceTypeForConfig.ACM;	// #24820 2012.12.23 yyagi 初期値はACM | #31927 2013.8.25 yyagi OSにより初期値変更
...
//			this.nASIOBufferSizeMs = 0;					// #24820 2012.12.25 yyagi 初期値は0(自動設定)
	...
	//------------------------------
			...
			sw.WriteLine( "SoundDeviceType={0}", (int)TJAPlayer3.stageコンフィグ.actList.iSystemSoundType.n現在選択されている項目番号);
			sw.WriteLine();
