			...
			if( !base.b活性化してない )
			{
                for( int i = 0; i < 2; i++ )
                {
                    ...
                    if( TJAPlayer3.Tx.Balloon_Combo[ i ] != null )
                    {
                        ...
                        if( this.ct進行[ i ].b進行中 )
                        {
                            TJAPlayer3.Tx.Balloon_Combo[ i ].t2D描画( TJAPlayer3.Skin.Game_Balloon_Combo_X[ i ], TJAPlayer3.Skin.Game_Balloon_Combo_Y[ i ] );
                            if( this.nCombo_渡[ i ] < 1000 ) //2016.08.23 kairera0467 仮実装。
                            {
                                ...
                                TJAPlayer3.Tx.Balloon_Number_Combo.t2D描画( TJAPlayer3.Skin.Game_Balloon_Combo_Text_X[ i ], TJAPlayer3.Skin.Game_Balloon_Combo_Text_Y[ i ], new Rectangle( 0, 54, 77, 32 ) );
                            }
                            else
                            {
                                ...
                                TJAPlayer3.Tx.Balloon_Number_Combo.t2D描画( TJAPlayer3.Skin.Game_Balloon_Combo_Text_Ex_X[ i ], TJAPlayer3.Skin.Game_Balloon_Combo_Text_Ex_Y[ i ], new Rectangle( 0, 54, 77, 32 ) );
                            }
                        }
                    }
                }
			...
			foreach( char ch in str )
			{
				for( int i = 0; i < this.st小文字位置.Length; i++ )
				{
					if( this.st小文字位置[ i ].ch == ch )
					{
						...
						{
                            TJAPlayer3.Tx.Balloon_Number_Combo.t2D描画( x, y, rectangle );
						}
						break;
					}
				}
