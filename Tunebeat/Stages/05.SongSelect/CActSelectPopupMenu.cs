		...
		public int t進行描画()
		{
			if ( !base.b活性化してない && this.bIsActivePopupMenu )
			{
				if ( this.bキー入力待ち )
				{
					...
					if ( ( TJAPlayer3.Input管理.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_RSHIFT ) || TJAPlayer3.Input管理.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_LSHIFT ) ) &&
					...
					#endregion
					#region [ キー入力: キャンセル ]
					else if (TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_ESCAPE)
						&& this.bEsc有効 )
					{	// キャンセル
						TJAPlayer3.Skin.sound取消音.t再生する();
						tCancel();
					...
					#endregion
					#region [ キー入力: 前に移動 ]
					this.ctキー反復用.Up.tキー反復( TJAPlayer3.Input管理.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_UP ), new CCounter.DGキー処理( this.t前に移動 ) );
					...
					#endregion
					#region [ キー入力: 次に移動 ]
					this.ctキー反復用.Down.tキー反復( TJAPlayer3.Input管理.IsPushingKey( (int) DxLibDLL.DX.KEY_INPUT_DOWN ), new CCounter.DGキー処理( this.t次に移動 ) );
				...
				{
                    TJAPlayer3.Tx.Menu_Title.t2D描画( 160, 40 );
				}
				#endregion
				#region [ ソートメニュータイトル描画 ]
				int x = 240, y = 44;
				stqMenuTitle.txName.t2D描画( x, y );
				#endregion
				#region [ カーソル描画 ]
				if ( TJAPlayer3.Tx.Menu_Highlight != null )
				{
                    ...
                    TJAPlayer3.Tx.Menu_Highlight.t2D描画( curX, curY, new Rectangle( 0, 0, 16, 32 ) );
					curX += 0x10;
                    ...
                    TJAPlayer3.Tx.Menu_Highlight.t2D描画( curX, curY, new Rectangle( 0x10, 0, 16, 32 ) );
				}
				#endregion
				#region [ ソート候補文字列描画 ]
				for ( int i = 0; i < lciMenuItems.Length; i++ )
				{
					...
					if ( lciMenuItems[ i ].txName != null )
					{
						...
						lciMenuItems[ i ].txName.t2D描画( 180, 77 + i * 32 );
					}
