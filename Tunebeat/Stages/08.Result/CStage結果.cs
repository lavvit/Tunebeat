	...
	internal class CStage結果 : CStage
	{
		...
		public override int On進行描画()
		{
			if (!base.b活性化してない)
			{
				...
				{
					TJAPlayer3.Tx.Result_Background.t2D描画(0, 0);
				}
				if (this.ct登場用.b進行中 && (TJAPlayer3.Tx.Result_Header != null))
				{
					...
					num = ((int)(TJAPlayer3.Tx.Result_Header.szテクスチャサイズ.Height * num3)) - TJAPlayer3.Tx.Result_Header.szテクスチャサイズ.Height;
				}
				else
				{
					num = 0;
				...
				for (int i = 0; i < TJAPlayer3.ConfigIni.nPlayerCount; i++)
				{
					...
					{
						TJAPlayer3.Tx.NamePlate[i].t2D描画(TJAPlayer3.Skin.Result_NamePlate_X[i], TJAPlayer3.Skin.Result_NamePlate_Y[i]);
					}
				}
				#endregion
				...
				//					if ( CDTXMania.Input管理.Keyboard.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_F12 ) &&
