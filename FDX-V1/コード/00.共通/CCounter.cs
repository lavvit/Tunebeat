...
namespace FDK
{
	...
	public class CCounter
	{
		...
		public CCounter(int n開始値, int n終了値, int n間隔ms, object timer)
			: this()
		{
		...
		public CCounter(double db開始値, double db終了値, double db間隔, object timer)
			: this()
		{
		...
		public void t開始(int n開始値, int n終了値, int n間隔ms, object timer)
		{
			this.n開始値 = n開始値;
			this.n終了値 = n終了値;
			this.n間隔ms = n間隔ms;
			this.n現在の経過時間ms = DxLibDLL.DX.GetNowCount();
			this.n現在の値 = n開始値;
		}
		...
		/// <param name="timer">カウントに使用するタイマ。</param>
		public void t開始(double db開始値, double db終了値, double db間隔, object timer)
		{
			this.db開始値 = db開始値;
			this.db終了値 = db終了値;
			this.db間隔 = db間隔;
			this.db現在の経過時間 = DxLibDLL.DX.GetNowCount();
			this.db現在の値 = db開始値;
		}
		...
		public void t進行db()
		{
			if ((this.db現在の経過時間 != -1))
			{
				double num = DxLibDLL.DX.GetNowCount();
			...
			if (bキー押下)
			{
				switch (this.n現在の値)
				{
					...
					case n2回目:
						...
						if ((DxLibDLL.DX.GetNowCount() - this.n現在の経過時間ms) > 200)
						{
							tキー処理();
							this.n現在の経過時間ms = DxLibDLL.DX.GetNowCount();
							this.n現在の値 = n3回目以降;
					...
					case n3回目以降:
						...
						if ((DxLibDLL.DX.GetNowCount() - this.n現在の経過時間ms) > 30)
						{
							tキー処理();
							this.n現在の経過時間ms = DxLibDLL.DX.GetNowCount();
						}
		...
		private object timerdb;
		private double n間隔ms;
		private double db間隔;
		//-----------------
		#endregion
	}
