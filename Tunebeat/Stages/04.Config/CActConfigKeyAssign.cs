	...
	internal class CActConfigKeyAssign : CActivity
	{
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
				...
				else if( ( TJAPlayer3.Input管理.IsPushedKey( (int)DxLibDLL.DX.KEY_INPUT_DELETE ) && ( this.n現在の選択行 >= 0 ) ) && ( this.n現在の選択行 <= 15 ) )
				{
					TJAPlayer3.Skin.sound決定音.t再生する();
				...
				if(TJAPlayer3.Tx.Menu_Highlight != null )
				{
					...
					for( int j = 0; j < 14; j++ )
					{
                        TJAPlayer3.Tx.Menu_Highlight.t2D描画( num2, num3, rectangle );
						num2 += 0x10;
					}
				...
				{
                    TJAPlayer3.Tx.Config_KeyAssign.t2D描画( 0x185, 0xd7 );
				}
			}
			return 0;
		}
		...
		private bool tキーチェックとアサイン_Keyboard()
		{
			...
			{
				if( i != (int)DxLibDLL.DX.KEY_INPUT_ESCAPE&&
					i != (int)DxLibDLL.DX.KEY_INPUT_RETURN &&
					i != (int)DxLibDLL.DX.KEY_INPUT_UP &&
					i != (int)DxLibDLL.DX.KEY_INPUT_DOWN &&
					i != (int)DxLibDLL.DX.KEY_INPUT_LEFT &&
					i != (int)DxLibDLL.DX.KEY_INPUT_RIGHT &&
					i != (int)DxLibDLL.DX.KEY_INPUT_DELETE &&
					 TJAPlayer3.Input管理.IsPushedKey( i ) )
				{
					TJAPlayer3.Skin.sound決定音.t再生する();
