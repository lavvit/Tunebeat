    ...
    internal class CAct演奏Drums風船 : CActivity
    {
        ...
        public int On進行描画(int n連打ノルマ, int n連打数, int player)
        {
            ...
            if (n連打数 > 0)
            {
                ...
                //    CDTXMania.Tx.Chara_Balloon_Breaking.t2D描画(CDTXMania.app.Det進行Loopice, CDTXMania.Skin.Game_Chara_Balloon_X[player], CDTXMania.Skin.Game_Chara_Balloon_Y[player]);
                for (int j = 0; j < 5; j++)
                {
                    if (n残り打数[j] < n連打数)
                    {
                        if (TJAPlayer3.Tx.Balloon_Breaking[j] != null)
                            TJAPlayer3.Tx.Balloon_Breaking[j].t2D描画(TJAPlayer3.Skin.Game_Balloon_Balloon_X[player] + (this.ct風船ふきだしアニメ.n現在の値 == 1 ? 3 : 0), TJAPlayer3.Skin.Game_Balloon_Balloon_Y[player]);
                        break;
                    }
                }
                //1P:31 2P:329
                if (TJAPlayer3.Tx.Balloon_Balloon != null)
                    TJAPlayer3.Tx.Balloon_Balloon.t2D描画(TJAPlayer3.Skin.Game_Balloon_Balloon_Frame_X[player], TJAPlayer3.Skin.Game_Balloon_Balloon_Frame_Y[player]);
            ...
            if (n連打数 == 0 && TJAPlayer3.stage演奏ドラム画面.actChara.b風船連打中)
            {
                ...
                //        CDTXMania.Tx.Chara_Balloon_Broken.t2D描画(CDTXMania.app.Det進行Loopice, 240, 140 - nY);
                ...
                for (int i = 0; i < this.st文字位置.Length; i++)
                {
                    if (this.st文字位置[i].ch == ch)
                    {
                        ...
                        if (TJAPlayer3.Tx.Balloon_Number_Roll != null)
                        {
                            ...
                            TJAPlayer3.Tx.Balloon_Number_Roll.t2D拡大率考慮下基準描画(x - (((TJAPlayer3.Skin.Game_Balloon_Number_Padding + 2) * n桁数) / 2), y, rectangle);
                        }
                        break;
                    }
                }
        ...
        public void tEnd()
        {
            this.ct風船終了 = new CCounter(0, 80, 10, null);
        }
    }
}
