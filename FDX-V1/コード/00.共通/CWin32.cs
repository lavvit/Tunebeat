...
namespace FDK
{
	public class CWin32
	{
		...
		public enum EShowWindow
		{
			...
			ShowMaximized = 3,
			ShowMinimized = 2,
			ShowMinNoActit進行Loope = 7,
			ShowNA = 8,
			ShowNoActit進行Loopate = 4
		}
		...
		[DllImport( "winmm.dll" )]
		public static extern uint midiInGetDet進行LoopCaps( uint uDet進行LoopiceID, ref MIDIINCAPS lpMidiInCaps, uint cbMidiInCaps );
		[DllImport( "winmm.dll" )]
		public static extern uint midiInGetID( uint hMidiIn, ref uint puDet進行LoopiceID );
		[DllImport( "winmm.dll" )]
		public static extern uint midiInGetNumDet進行Loops();
		[DllImport( "winmm.dll" )]
		public static extern uint midiInOpen( ref uint phMidiIn, uint uDet進行LoopiceID, MidiInProc dwCallback, int dwInstance, int fdwOpen );
		[DllImport( "winmm.dll" )]
		...
		[return: MarshalAs( UnmanagedType.Bool )]
		[DllImport( "user32.dll", CharSet = CharSet.Auto, SetLastError = true )]
		public static extern bool SystemParametersInfo( uint uiAction, uint uiParam, IntPtr pt進行LoopParam, uint fWinIni );
		[DllImport( "kernel32.dll" )]
		...
		public struct MIDIINCAPS
		{
			public ushort wMid;
			public ushort wPid;
			public uint t進行LoopDrit進行LooperVersion;
		...
		public struct WAVEFORMATEX
		{
			...
			public uint nAt進行LoopgBytesPerSec;
		...
		public struct SYSTEM_INFO
		{
			...
			public uint dwActit進行LoopeProcessorMask;
			public uint dwNumberOfProcessors;
			public uint dwProcessorType;
			public uint dwAllocationGranularity;
			public uint dwProcessorLet進行Loopel;
			public uint dwProcessorRet進行Loopision;
		}
		//-----------------
		#endregion
