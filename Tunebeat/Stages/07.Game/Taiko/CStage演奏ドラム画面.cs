...
namespace TJAPlayer3
{
	internal class CStage演奏ドラム画面 : CStage演奏画面共通
	{
		...
		public override void On活性化()
		{
            ...
            ////this.actChara.ct通常モーション = new CCounter( 0, this.actChara.arモーション番号.Length - 1, 0.07, null );
            ...
            //this.actDancer.ctモブ = new CCounter( 1.0, 16.0, ((60.0 / CDTXMania.stage演奏ドラム画面.actPlayInfo.dbBPM / 16.0 )), null );
            ...
            this.ct演奏用タイマー = new CCounter((double)(-1000 * (TJAPlayer3.ConfigIni.n演奏速度 / 20.0)), (double)int.MaxValue, 0.001, TJAPlayer3.Timer);
            //try
            //{
    ...
    //            this.tx判定数表示パネル = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\7_Paramater Panel.png" ) );
			    ...
			    {
			        this.soundRed = new CSound( CSkin.Path( @"Sounds\Taiko\dong.ogg" ));
			        this.soundBlue = new CSound( CSkin.Path( @"Sounds\Taiko\ka.ogg" ));
			        this.soundAdlib = new CSound( CSkin.Path(@"Sounds\Taiko\Adlib.ogg"));
			    }
    ...
    //            CDTXMania.tテクスチャの解放( ref this.txPlayerNumber);
		...
		protected override E判定 tチップのヒット処理(double nHitTime, CDTX.CChip pChip, bool bCorrectLane )
		{
		...
		protected override void tチップのヒット処理_BadならびにTight時のMiss( E楽器パート part, int nコース)
		{
			this.tチップのヒット処理_BadならびにTight時のMiss( part, 0, E楽器パート.DRUMS, nコース);
		}
		private bool tドラムヒット処理(double nHitTime, Eパッド type, CDTX.CChip pChip, bool b両手入力, int nPlayer )
		{
            int nInput = 0;
        ...
        public int[] Keys = new int[] { DX.KEY_INPUT_F, DX.KEY_INPUT_J, DX.KEY_INPUT_D, DX.KEY_INPUT_K, DX.KEY_INPUT_C, DX.KEY_INPUT_N, DX.KEY_INPUT_X, DX.KEY_INPUT_M ,DX.KEY_INPUT_G, DX.KEY_INPUT_H, DX.KEY_INPUT_S, DX.KEY_INPUT_L};
		protected override void t入力処理_ドラム()
		{
		    var nInputAdjustTimeMs = TJAPlayer3.ConfigIni.nInputAdjustTimeMs + ct演奏用タイマー.db現在の値;
            for (int i = 0; i < Keys.Length; i++)
            {
                if (TJAPlayer3.Input管理.IsPushedKey(Keys[i]))
                {
                    {
                        int nPad = TJAPlayer3.Input管理.IsPushingKey(DX.KEY_INPUT_F) ? 12 : TJAPlayer3.Input管理.IsPushingKey(DX.KEY_INPUT_J) ? 13 : TJAPlayer3.Input管理.IsPushingKey(DX.KEY_INPUT_D) ? 14 : TJAPlayer3.Input管理.IsPushingKey(DX.KEY_INPUT_K) ? 15 :
                             TJAPlayer3.Input管理.IsPushingKey(DX.KEY_INPUT_C) ? 16 : TJAPlayer3.Input管理.IsPushingKey(DX.KEY_INPUT_N) ? 17 : TJAPlayer3.Input管理.IsPushingKey(DX.KEY_INPUT_X) ? 18 : TJAPlayer3.Input管理.IsPushingKey(DX.KEY_INPUT_M) ? 19 : 12;
                        switch (i)
                        {
                            case 0: nPad = 12; break;
                            case 1: nPad = 13; break;
                            case 2: nPad = 14; break;
                            case 3: nPad = 15; break;
                            //2P
                            case 4: nPad = 16; break;
                            case 5: nPad = 17; break;
                            case 6: nPad = 18; break;
                            case 7: nPad = 19; break;

                            //1P 2
                            case 8: nPad = 12; break;
                            case 9: nPad = 13; break;
                            case 10: nPad = 14; break;
                            case 11: nPad = 15; break;
                        }
                        double nTime = nInputAdjustTimeMs;
                        ...
                        CDTX.CChip chipNoHit = chip現在処理中の連打チップ[nUsePlayer] == null ? r指定時刻に一番近い未ヒットChipを過去方向優先で検索する((long)nTime, nUsePlayer) : r指定時刻に一番近い未ヒットChipを過去方向優先で検索する((long)nTime, nUsePlayer);
                            ...
                            case Eパッド.LRed2P:
                                ...
                                //-----------------------------
                                {
                                    ...
                                    if (e判定 != E判定.Miss && (chipNoHit.nチャンネル番号 == 0x13 || chipNoHit.nチャンネル番号 == 0x1A) && TJAPlayer3.ConfigIni.b大音符判定)
                                    {
                                        ...
                                        {
                                            double time = chipNoHit.n発声時刻ms - ct演奏用タイマー.db現在の値;
                                            if (time <= 110)
                                            {
                                                chipNoHit.nProcessTime = (int)ct演奏用タイマー.db現在の値;
                                        ...
                                        else if (chipNoHit.eNoteState == ENoteState.wait)
                                        {
                                            ...
                                            if (this.nWaitButton == 1 && time <= 110 && chipNoHit.nProcessTime + nWaitTime > (int)ct演奏用タイマー.db現在の値)
                                            {
                                                this.tドラムヒット処理(nTime, Eパッド.LRed, chipNoHit, true, nUsePlayer);
                                                bHitted = true;
                                                this.nWaitButton = 0;
                                            ...
                                            else if (this.nWaitButton == 2 && time <= 110 && chipNoHit.nProcessTime + nWaitTime < (int)ct演奏用タイマー.db現在の値)
                                            {
                                                this.tドラムヒット処理(nTime, Eパッド.LRed, chipNoHit, false, nUsePlayer);
                                                bHitted = true;
                                                this.nWaitButton = 0;
                                ...
                                //-----------------------------
                                {
                                    ...
                                    if (e判定 != E判定.Miss && (chipNoHit.nチャンネル番号 == 0x13 || chipNoHit.nチャンネル番号 == 0x1A) && TJAPlayer3.ConfigIni.b大音符判定)
                                    {
                                        ...
                                        else if (chipNoHit.eNoteState == ENoteState.wait)
                                        {
                                            double time = chipNoHit.n発声時刻ms - ct演奏用タイマー.db現在の値;
                                            int nWaitTime = TJAPlayer3.ConfigIni.n両手判定の待ち時間;
                                            if (this.nWaitButton == 2 && time <= 110 && chipNoHit.nProcessTime + nWaitTime > (int)ct演奏用タイマー.db現在の値)
                                            {
                                                this.tドラムヒット処理(nTime, Eパッド.RRed, chipNoHit, true, nUsePlayer);
                                                bHitted = true;
                                                this.nWaitButton = 0;
                        ...
                        if (TJAPlayer3.ConfigIni.bTight && !b連打中[nUsePlayer]) // 18/8/13 - 連打時にこれが発動すると困る!!! (AioiLight)
                            this.tチップのヒット処理_BadならびにTight時のMiss(E楽器パート.DRUMS, 0, E楽器パート.TAIKO, chipNoHit.nコース);
                        //-----------------------------
                        #endregion
                    }
                }
		...
		protected override void t進行描画_チップ_Taiko( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
        {
            ...
            if( pChip.b可視 )
            {
				...
				{
                    double nPlayTime = ct演奏用タイマー.db現在の値;
                    ...
                    if( pChip.dbSCROLL_Y != 0.0 )
                    {
                        ...
                        if( TJAPlayer3.ConfigIni.eScrollMode == EScrollMode.Normal )
                            y += (int) ( ( ( pChip.n発声時刻ms - ct演奏用タイマー.db現在の値 ) * pChip.dbBPM * pChip.dbSCROLL_Y * ( this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.5 ) ) / 628.7 );
                        ...
                        if( TJAPlayer3.Tx.Notes != null )
                        {
                            ...
                            switch ( pChip.nチャンネル番号 )
                            {
                                case 0x11:
                                    ...
                                    if( TJAPlayer3.Tx.Notes != null )
                                    {
                                        if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                            TJAPlayer3.Tx.Notes.t2D描画(  x, y, new Rectangle( 130, num9, 130, 130 ) );
                                ...
                                case 0x12:
                                    ...
                                    {
                                        if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                            TJAPlayer3.Tx.Notes.t2D描画(  x, y, new Rectangle( 260, num9, 130, 130) );
                                        TJAPlayer3.Tx.SENotes.t2D描画(  x - 2, y + nSenotesY, new Rectangle( 0, 30 * pChip.nSenote, 136, 30 ) );
                                        //CDTXMania.act文字コンソール.tPrint( x + 60, y + 140, C文字コンソール.Eフォント種別.白, pChip.nSenote.ToString() );
                                    }
                                    nLane = 1;
                                ...
                                case 0x13:
                                    if( TJAPlayer3.Tx.Notes != null )
                                    {
                                        if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                        {
                                            TJAPlayer3.Tx.Notes.t2D描画(  x, y, new Rectangle( 390, num9, 130, 130 ) );
                                            //CDTXMania.Tx.Notes.t3D描画(  mat, new Rectangle( 390, num9, 130, 130 ) );
                                        }
                                        TJAPlayer3.Tx.SENotes.t2D描画(  x - 2, y + nSenotesY, new Rectangle( 0, 30 * pChip.nSenote, 136, 30 ) );
                                        //CDTXMania.act文字コンソール.tPrint( x + 60, y + 140, C文字コンソール.Eフォント種別.白, pChip.nSenote.ToString() );
                                    }
                                ...
                                case 0x14:
                                    ...
                                    {
                                        if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                            TJAPlayer3.Tx.Notes.t2D描画(  x, y, new Rectangle( 520, num9, 130, 130 ) );
                                        TJAPlayer3.Tx.SENotes.t2D描画(  x - 2, y + nSenotesY, new Rectangle( 0, 30 * pChip.nSenote, 136, 30 ) );
                                        //CDTXMania.act文字コンソール.tPrint( x + 60, y + 140, C文字コンソール.Eフォント種別.白, pChip.nSenote.ToString() );
                                    }
                                    nLane = 1;
                                ...
                                case 0x1A:
                                    if( TJAPlayer3.Tx.Notes != null )
                                    {
                                        if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                        {
                                            if( nPlayer == 0 )
                                            {
                                                TJAPlayer3.Tx.Notes_Arm.t2D上下反転描画(  x + 25, ( y + 74 ) + nHand );
                                                TJAPlayer3.Tx.Notes_Arm.t2D上下反転描画(  x + 60, ( y + 104 ) - nHand );
                                            }
                                            else if( nPlayer == 1 )
                                            {
                                                TJAPlayer3.Tx.Notes_Arm.t2D描画(  x + 25, ( y - 44 ) + nHand );
                                                TJAPlayer3.Tx.Notes_Arm.t2D描画(  x + 60, ( y - 14 ) - nHand );
                                            }
                                            TJAPlayer3.Tx.Notes.t2D描画(  x, y, new Rectangle( 1690, num9, 130, 130 ) );
                                            //CDTXMania.Tx.Notes.t3D描画(  mat, new Rectangle( 390, num9, 130, 130 ) );
                                        }
                                        TJAPlayer3.Tx.SENotes.t2D描画(  x - 2, y + nSenotesY, new Rectangle( 0, 390, 136, 30 ) );
                                    }
                                    break;
                                ...
                                case 0x1B:
                                    if( TJAPlayer3.Tx.Notes != null )
                                    {
                                        if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                        {
                                            ...
                                            TJAPlayer3.Tx.Notes.t2D描画(  x, y, new Rectangle( 1820, num9, 130, 130 ) );
                                        }
                                        TJAPlayer3.Tx.SENotes.t2D描画(  x - 2, y + nSenotesY, new Rectangle( 0, 420, 136, 30 ) );
                                    }
                                    nLane = 1;
                                    break;
		...
		protected override void t進行描画_チップ_Taiko連打( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
        {
            ...
            if( pChip.b可視 )
            {
                ...
                {
                    if( pChip.nノーツ出現時刻ms != 0 && ( ct演奏用タイマー.db現在の値 < pChip.n発声時刻ms - pChip.nノーツ出現時刻ms ) )
                        pChip.bShow = false;
                    else if( pChip.nノーツ出現時刻ms != 0 && pChip.nノーツ移動開始時刻ms != 0 )
                        pChip.bShow = true;

                    if( pChip.nノーツ移動開始時刻ms != 0 && ( ct演奏用タイマー.db現在の値 < pChip.n発声時刻ms - pChip.nノーツ移動開始時刻ms ) )
                    {
                ...
                {
                    if( pChip.nノーツ出現時刻ms != 0 && ( ct演奏用タイマー.db現在の値 < n先頭発声位置 - pChip.nノーツ出現時刻ms ) )
                        pChip.bShow = false;
                    else
                        pChip.bShow = true;
                    ...
                    if( pChip.nノーツ移動開始時刻ms != 0 && ( ct演奏用タイマー.db現在の値 < n先頭発声位置 - pChip.nノーツ移動開始時刻ms ) )
                    {
                ...
                if( ( 1400 > x ) && pChip.bShow )
                {
                    if( TJAPlayer3.Tx.Notes != null )
                    {
                        ...
                        var effectedColor = System.Drawing.Color.FromArgb(255, (int)(f減少するカラー * 255f), (int)(f減少するカラー * 255f));
                        var normalColor = System.Drawing.Color.FromArgb(255, 255, 255);
                        ...
                        if ( pChip.nチャンネル番号 == 0x15 ) //連打(小)
                        {
                            ...
                            if ( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                            {
                                ...
                                //CDTXMania.Tx.Notes.color4 = System.Drawing.Color.FromArgb(1.0f, f減少するカラー, f減少するカラー);
                                ...
                                //CDTXMania.Tx.Notes.color4 = System.Drawing.Color.FromArgb(1.0f, 1.0f, 1.0f); //先端シンボルは色を変えない
                                ...
                                TJAPlayer3.Tx.Notes.t2D描画(x + 64, y, new Rectangle(781, 0, 128, 130));
                                TJAPlayer3.Tx.Notes.vc拡大縮小倍率.X = 1.0f;
                                TJAPlayer3.Tx.Notes.t2D描画(x末端 + f末端ノーツのテクスチャ位置調整, y, 0, new Rectangle(910, num9, 130, 130));
                                if (TJAPlayer3.Skin.Game_RollColorMode == CSkin.RollColorMode.All)
                                    TJAPlayer3.Tx.Notes.color4 = effectedColor;
                                else
                                    TJAPlayer3.Tx.Notes.color4 = normalColor;
                                
                                TJAPlayer3.Tx.Notes.t2D描画(x, y, 0, new Rectangle(650, num9, 130, 130));
                                TJAPlayer3.Tx.Notes.color4 = normalColor;
                                #endregion
                            }
                            TJAPlayer3.Tx.SENotes.vc拡大縮小倍率.X = index - 44;
                            TJAPlayer3.Tx.SENotes.t2D描画( x + 90, y + nSenotesY, new Rectangle( 60, 240, 1, 30 ) );
                            TJAPlayer3.Tx.SENotes.vc拡大縮小倍率.X = 1.0f;
                            TJAPlayer3.Tx.SENotes.t2D描画( x + 30, y + nSenotesY, new Rectangle(0, 240, 60, 30));
                            TJAPlayer3.Tx.SENotes.t2D描画(x, y + nSenotesY, new Rectangle(0, 30 * pChip.nSenote, 136, 30));
                        }
                        if( pChip.nチャンネル番号 == 0x16 )
                        {
                            ...
                            if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                            {
                                ...
                                TJAPlayer3.Tx.Notes.t2D描画(x + 64, y, new Rectangle(1171, 0, 128, 130));

                                TJAPlayer3.Tx.Notes.vc拡大縮小倍率.X = 1.0f;
                                TJAPlayer3.Tx.Notes.t2D描画(x末端 + f末端ノーツのテクスチャ位置調整, y, 0, new Rectangle(1300, num9, 130, 130));
                                if (TJAPlayer3.Skin.Game_RollColorMode == CSkin.RollColorMode.All)
                                    TJAPlayer3.Tx.Notes.color4 = effectedColor;
                                else
                                    TJAPlayer3.Tx.Notes.color4 = normalColor;

                                TJAPlayer3.Tx.Notes.t2D描画(x, y, new Rectangle(1040, num9, 130, 130));
                                TJAPlayer3.Tx.Notes.color4 = normalColor;
                                #endregion
                            }
                            TJAPlayer3.Tx.SENotes.vc拡大縮小倍率.X = index - 70;
                            TJAPlayer3.Tx.SENotes.t2D描画( x + 116, y + nSenotesY, new Rectangle( 60, 240, 1, 30 ) );
                            TJAPlayer3.Tx.SENotes.vc拡大縮小倍率.X = 1.0f;
                            TJAPlayer3.Tx.SENotes.t2D描画(x + 56, y + nSenotesY, new Rectangle(0, 240, 60, 30));
                            TJAPlayer3.Tx.SENotes.t2D描画(x - 2, y + nSenotesY, new Rectangle(0, 30 * pChip.nSenote, 136, 30));
                        }
                        if( pChip.nチャンネル番号 == 0x17 )
                        {
                            if( pChip.n発声時刻ms < ct演奏用タイマー.db現在の値 && pChip.nノーツ終了時刻ms > ct演奏用タイマー.db現在の値 )
                                x = 349;
                            if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                TJAPlayer3.Tx.Notes.t2D描画( x, y, new Rectangle( 1430, num9, 260, 130 ) );
                        ...
                        if( pChip.nチャンネル番号 == 0x18 )
                        {
                            ...
                            {
                                TJAPlayer3.Tx.SENotes.t2D描画(x + 56, y + nSenotesY, new Rectangle( 58, 270, 78, 30 ) );
                            }
                        }
                    }
                }
		...
		protected override void t進行描画_チップ_小節線( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
		{
			...
			{
                if ( x >= 310 ) TJAPlayer3.act文字コンソール.tPrint(x + 8, y - 26, C文字コンソール.Eフォント種別.白, pChip.n整数値_内部番号.ToString());
			}
			if ( ( pChip.b可視 ) && (TJAPlayer3.Tx.Bar != null ) )
			{
                if( x >= 0 )
                {
                    if( pChip.bBranch )
                    {
                        TJAPlayer3.Tx.Bar_Branch.t2D描画(x, TJAPlayer3.Skin.nScrollFieldY[nPlayer], new Rectangle( 0, 0, 3, 130 ) );
                    }
                    else
                    {
                        TJAPlayer3.Tx.Bar.t2D描画( x, TJAPlayer3.Skin.nScrollFieldY[nPlayer], new Rectangle( 0, 0, 3, 130 ) );
                    }
                }
			}
		}
        ...
        public void t全体制御メソッド()
        {
            for ( int i = 0; i < TJAPlayer3.ConfigIni.nPlayerCount; i++ )
            {
                if( this.chip現在処理中の連打チップ[ i ] != null )
                {
                    ...
                    {
                        //if (this.chip現在処理中の連打チップ.n発声時刻ms <= (int)ct演奏用タイマー.db現在の値 && this.chip現在処理中の連打チップ.nノーツ終了時刻ms >= (int)ct演奏用タイマー.db現在の値)
                        if( this.chip現在処理中の連打チップ[ i ].n発声時刻ms <= (int)ct演奏用タイマー.db現在の値 && this.chip現在処理中の連打チップ[ i ].nノーツ終了時刻ms + 500 >= (int)ct演奏用タイマー.db現在の値)
                        {
                            this.actBalloon.On進行描画( this.chip現在処理中の連打チップ[ i ].nBalloon, this.n風船残り[ i ], i );
                        }
                        else this.n現在の連打数[i] = 0;
                    }
                }
            }
            ...
            //CDTX.CChip chipNoHit = this.r指定時刻に一番近い未ヒットChip((int)ct演奏用タイマー.db現在の値, 0);
            for( int i = 0; i < TJAPlayer3.ConfigIni.nPlayerCount; i++ )
            {
                CDTX.CChip chipNoHit = r指定時刻に一番近い未ヒットChipを過去方向優先で検索する((long)ct演奏用タイマー.db現在の値, i );

                if( chipNoHit != null && ( chipNoHit.nチャンネル番号 == 0x13 || chipNoHit.nチャンネル番号 == 0x14 || chipNoHit.nチャンネル番号 == 0x1A || chipNoHit.nチャンネル番号 == 0x1B ) )
                {
                    double timeC = chipNoHit.n発声時刻ms - ct演奏用タイマー.db現在の値;
                    int nWaitTime = TJAPlayer3.ConfigIni.n両手判定の待ち時間;
                    if (chipNoHit.eNoteState == ENoteState.wait && timeC <= 110 && chipNoHit.nProcessTime + nWaitTime <= (int)ct演奏用タイマー.db現在の値)
                    {
            ...
            if( TJAPlayer3.Input管理.IsPushedKey((int)DxLibDLL.DX.KEY_INPUT_F1 ) )
            {
        ...
        private void t進行描画_リアルタイム判定数表示()
        {
            if( TJAPlayer3.ConfigIni.nPlayerCount == 1 ? ( TJAPlayer3.ConfigIni.bJudgeCountDisplay && !TJAPlayer3.ConfigIni.b太鼓パートAutoPlay ) : false )
            {
                ...
                if( TJAPlayer3.Tx.Judge_Meter != null )
                    TJAPlayer3.Tx.Judge_Meter.t2D描画( 0, 360 );
