	...
	internal class CActConfigList : CActivity
	{
		...
		public void t項目リストの設定_System()
		{
			...
			this.iSystemSoundType = new CItemList("SoundType", CItemList.Eパネル種別.通常, TJAPlayer3.ConfigIni.nSoundDeviceType,
				...
				new string[] { "DirectSound", "ASIO", "WASAPI(排他)", "WASAPI(共有)" });
...
//			this.tConfigIniへ記録する();
		...
		public int t進行描画( bool b項目リスト側にフォーカスがある )
		{
			...
			if( base.b初めての進行描画 )
			{
                this.nスクロール用タイマ値 = DX.GetNowCount();
			...
			for( int n行番号 = -4; n行番号 < 6; n行番号++ )		// n行番号 == 0 がフォーカスされている項目パネル。
			{
				...
				{
                    listMenu[nItem].txMenuItemRight.t2D描画(x + 20 + TJAPlayer3.Skin.Config_ItemText_Correction_X, y + 12 + TJAPlayer3.Skin.Config_ItemText_Correction_Y);
				}
				else
				{
				...
				if ( b強調 )
				{
				    using (var bmpStr = prvFont.DrawPrivateFont(strParam, Color.Black, Color.White, Color.Yellow, Color.OrangeRed))
				    {
				        ...
				        {
				            txStr.t2D描画( x + 400 + TJAPlayer3.Skin.Config_ItemText_Correction_X, y + 12 + TJAPlayer3.Skin.Config_ItemText_Correction_Y );
				        }
				    }
				}
				else
				{
					...
					listMenu[ nItem ].txParam.t2D描画(  x + 400 + TJAPlayer3.Skin.Config_ItemText_Correction_X, y + 12 + TJAPlayer3.Skin.Config_ItemText_Correction_Y );
				}
				//-----------------
				#endregion
			...
			if( this.b項目リスト側にフォーカスがある && ( this.n目標のスクロールカウンタ == 0 ) )
			{
				...
				{
                    TJAPlayer3.Tx.Config_Arrow.t2D描画( x, y_upper, new Rectangle( 0, 0, 0x40, 0x18 ) );
                    TJAPlayer3.Tx.Config_Arrow.t2D描画( x, y_lower, new Rectangle( 0, 0x18, 0x40, 0x18 ) );
				}
			}
			//-----------------
			#endregion
...
//		private CItemToggle iSystemGuitar;
		...
		public CItemList iSystemSoundType;					// #24820 2013.1.3 yyagi
