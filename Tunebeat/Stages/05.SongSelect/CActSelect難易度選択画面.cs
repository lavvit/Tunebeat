	...
	internal class CActSelect難易度選択画面 : CActivity
	{
        ...
        public void t次に移動()
		{
			if( TJAPlayer3.stage選曲.r現在選択中の曲 != null )
			{
                ...
                this.ct移動 = new CCounter( 1, 710, 1, null );
			}
		}
		public void t前に移動()
		{
		...
		public override void OnManagedリソースの作成()
		{
            ...
            this.soundSelectAnnounce = new CSound( CSkin.Path( @"Sounds\DiffSelect.ogg" ));
		...
		public override int On進行描画()
		{
			...
			else
			{
                ...
                //キー操作
                if( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_RIGHT ) )
                {
                    this.t次に移動();
                }
                else if( TJAPlayer3.Input管理.IsPushedKey( (int) DxLibDLL.DX.KEY_INPUT_LEFT) )
                {
                    this.t前に移動();
                }
			...
			else
			{
				...
				for( int j = 0; j < (int)Difficulty.Total; j++ )
				{
                    ...
                    if( this.tx踏み台[ n描画順[ j ] ] != null )
                    {
                        ...
                        this.tx踏み台[ n描画順[ j ] ].t2D描画( n踏み台座標[ j ], 720 - this.tx踏み台[ n描画順[ j ] ].szテクスチャサイズ.Height + ( bEven ? 35 : 0 ) );
                    }
				}
            ...
            if( this.txヘッダー != null )
                this.txヘッダー.t2D描画( 0, 0 );
            if( this.txフッター != null )
                this.txフッター.t2D描画( 0, 720 - this.txフッター.szテクスチャサイズ.Height );

            if( this.tx説明背景 != null )
                this.tx説明背景.t2D描画( 340, 600 );
            if( this.tx説明1 != null )
                this.tx説明1.t2D描画( 340, 600 );
